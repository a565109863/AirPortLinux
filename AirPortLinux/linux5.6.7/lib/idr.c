...
next:
    bitmap = (struct ida_bitmap *)xas_find_marked(&xas, max / IDA_BITMAP_BITS, XA_FREE_MARK);
    ...
    if (xa_is_value(bitmap)) {
        ...
        bitmap = alloc;
        if (!bitmap)
            bitmap = (struct ida_bitmap *)kzalloc(sizeof(*bitmap), GFP_NOWAIT);
        if (!bitmap)
            goto alloc;
        bitmap->bitmap[0] = tmp;
    ...
    if (bitmap) {
        ...
        if (bit < BITS_PER_XA_VALUE) {
            bitmap = (struct ida_bitmap *)xa_mk_value(1UL << bit);
        } else {
            bitmap = alloc;
            if (!bitmap)
                bitmap = (struct ida_bitmap *)kzalloc(sizeof(*bitmap), GFP_NOWAIT);
...
alloc:
    xas_unlock_irqrestore(&xas, flags);
    alloc = (struct ida_bitmap *)kzalloc(sizeof(*bitmap), gfp);
    if (!alloc)
        return -ENOMEM;
...
void ida_free(struct ida *ida, unsigned int id)
{
    ...
    xas_lock_irqsave(&xas, flags);
    bitmap = (struct ida_bitmap *)xas_load(&xas);
