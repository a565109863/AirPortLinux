    ...
    do {
        ...
        if (id + n_groups > mc_groups_longs * BITS_PER_LONG) {
            ...
            if (mc_groups == &mc_group_start) {
                new_groups = (unsigned long *)kzalloc(nlen, GFP_KERNEL);
                if (!new_groups)
                    return -ENOMEM;
                mc_groups = new_groups;
                *mc_groups = mc_group_start;
            } else {
                new_groups = (unsigned long *)krealloc(mc_groups, nlen,
                              GFP_KERNEL);
                if (!new_groups)
                    return -ENOMEM;
                mc_groups = new_groups;
...
int genl_register_family(struct genl_family *family)
{
    ...
    if (family->maxattr && !family->parallel_ops) {
        family->attrbuf = (struct nlattr **)kmalloc_array(family->maxattr + 1,
          ...
          const struct genl_family *family, int flags, u8 cmd)
{
    ...
    hdr = (struct genlmsghdr *)nlmsg_data(nlh);
    hdr->cmd = cmd;
...
genl_family_rcv_msg_attrs_parse(const struct genl_family *family,
                ...
                bool parallel)
{
    enum netlink_validation validate = (enum netlink_validation)(ops->validate & no_strict_flag ?
                       NL_VALIDATE_LIBERAL :
                       NL_VALIDATE_STRICT);
                    ...
                    int hdrlen, struct net *net)
{
    ...
    info.nlhdr = nlh;
    info.genlhdr = (struct genlmsghdr *)nlmsg_data(nlh);
    info.userhdr = (struct genlmsghdr *)nlmsg_data(nlh) + GENL_HDRLEN;
    info.attrs = attrbuf;
    info.extack = extack;
...
static int ctrl_getfamily(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[CTRL_ATTR_FAMILY_NAME]) {
        ...
        name = (char *)nla_data(info->attrs[CTRL_ATTR_FAMILY_NAME]);
