...
{
    int ret;
    trace_rdev_tx_control_port(&rdev->wiphy, dev, (const u8 *)buf, len,
                   dest, proto, noencrypt);
    ret = rdev->ops->tx_control_port(&rdev->wiphy, dev, (const u8 *)buf, len,
                     dest, proto, noencrypt);
    trace_rdev_return_int(&rdev->wiphy, ret);
    return ret;
}
             ...
             const void *buf, size_t len)
{
    ...
    trace_rdev_probe_mesh_link(&rdev->wiphy, dev, dest, (const u8 *)buf, len);
    ret = rdev->ops->probe_mesh_link(&rdev->wiphy, dev, (const u8 *)buf, len);
    trace_rdev_return_int(&rdev->wiphy, ret);
    return ret;
}
