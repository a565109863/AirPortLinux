              ...
              struct genl_info *info)
{
    ...
    out->ftm.preamble = (enum nl80211_preamble)preamble;
...
int nl80211_pmsr_start(struct sk_buff *skb, struct genl_info *info)
{
    ...
    peers = nla_find((struct nlattr *)nla_data(reqattr), nla_len(reqattr),
    ...
    req = (struct cfg80211_pmsr_request *)kzalloc(struct_size(req, peers, count), GFP_KERNEL);
    if (!req)
        return -ENOMEM;
