...
__cfg80211_rdev_from_attrs(struct net *netns, struct nlattr **attrs)
{
    ...
    if (attrs[NL80211_ATTR_WDEV]) {
        u64 wdev_id = 1;// nla_get_u64(attrs[NL80211_ATTR_WDEV]);
        struct wireless_dev *wdev;
        bool found = false;
    ...
    if (attrs[NL80211_ATTR_IFINDEX]) {
        int ifindex = 1;// nla_get_u32(attrs[NL80211_ATTR_IFINDEX]);

        netdev = NULL;// __dev_get_by_index(netns, ifindex);
        if (netdev) {
...
{
    const u8 *data = NULL;// nla_data(attr);
    unsigned int len = 0;// nla_len(attr);
                  ...
                  struct wireless_dev **wdev)
{
    ...
    if (!cb->args[0]) {
        ...
        attrbuf = (struct nlattr **)kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf),
                  GFP_KERNEL);
        if (!attrbuf)
            return -ENOMEM;
                 ...
                 struct key_parse *k)
{
    ...
    if (tb[NL80211_KEY_DATA]) {
        k->p.key = (const u8 *)nla_data(tb[NL80211_KEY_DATA]);
        k->p.key_len = nla_len(tb[NL80211_KEY_DATA]);
    }

    if (tb[NL80211_KEY_SEQ]) {
        k->p.seq = (const u8 *)nla_data(tb[NL80211_KEY_SEQ]);
    ...
    if (tb[NL80211_KEY_TYPE])
        k->type = (enum nl80211_iftype)nla_get_u32(tb[NL80211_KEY_TYPE]);
    ...
    if (tb[NL80211_KEY_MODE])
        k->p.mode = (enum nl80211_key_mode)nla_get_u8(tb[NL80211_KEY_MODE]);
...
static int nl80211_parse_key_old(struct genl_info *info, struct key_parse *k)
{
    if (info->attrs[NL80211_ATTR_KEY_DATA]) {
        k->p.key = (const u8 *)nla_data(info->attrs[NL80211_ATTR_KEY_DATA]);
        k->p.key_len = nla_len(info->attrs[NL80211_ATTR_KEY_DATA]);
    }

    if (info->attrs[NL80211_ATTR_KEY_SEQ]) {
        k->p.seq = (const u8 *)nla_data(info->attrs[NL80211_ATTR_KEY_SEQ]);
    ...
    if (info->attrs[NL80211_ATTR_KEY_TYPE])
        k->type = (enum nl80211_iftype)nla_get_u32(info->attrs[NL80211_ATTR_KEY_TYPE]);
...
{
    struct nlattr *keys = (struct nlattr *)info->attrs[NL80211_ATTR_KEYS];
    ...
    result = (struct cfg80211_cached_keys *)kzalloc(sizeof(*result), GFP_KERNEL);
    if (!result)
...
#undef CMD
    ...
    case 9:
        ...
        if (rdev->wiphy.n_addresses > 1) {
            ...
            nla_nest_end(msg, (struct nlattr *)attr);
        }
...
{
    struct nlattr **tb = (struct nlattr **)kcalloc(NUM_NL80211_ATTR, sizeof(*tb), GFP_KERNEL);
    int ret;
...
{
    int idx = 0, ret;
    struct nl80211_dump_wiphy_state *state = NULL; // (void *)cb->args[0];
    struct cfg80211_registered_device *rdev;

    rtnl_lock();
    if (!state) {
        state = (struct nl80211_dump_wiphy_state *)kzalloc(sizeof(*state), GFP_KERNEL);
        if (!state) {
            rtnl_unlock();
            return -ENOMEM;
        }
...
{
    struct sk_buff *msg;
    struct cfg80211_registered_device *rdev = (struct cfg80211_registered_device *)info->user_ptr[0];
    struct nl80211_dump_wiphy_state state = {};
              ...
              struct cfg80211_chan_def *chandef)
{
    ...
    if (attrs[NL80211_ATTR_WIPHY_CHANNEL_TYPE]) {
        ...
        chandef->width =
            (enum nl80211_chan_width)nla_get_u32(attrs[NL80211_ATTR_CHANNEL_WIDTH]);
        if (attrs[NL80211_ATTR_CENTER_FREQ1])
    ...
    if (info->attrs[NL80211_ATTR_WIPHY_EDMG_CHANNELS]) {
        ...
        if (info->attrs[NL80211_ATTR_WIPHY_EDMG_BW_CONFIG])
            chandef->edmg.bw_config =
             (enum ieee80211_edmg_bw_config)nla_get_u8(info->attrs[NL80211_ATTR_WIPHY_EDMG_BW_CONFIG]);
    } else {
        chandef->edmg.bw_config = (enum ieee80211_edmg_bw_config)0;
...
{
    struct cfg80211_registered_device *rdev = (struct cfg80211_registered_device *)info->user_ptr[0];
    struct net_device *netdev = (struct net_device *)info->user_ptr[1];
...
{
    struct cfg80211_registered_device *rdev = (struct cfg80211_registered_device *)info->user_ptr[0];
    struct net_device *dev = (struct net_device *)info->user_ptr[1];
    ...
    bssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_set_wiphy(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_WIPHY_TX_POWER_SETTING]) {
        ...
        type = (enum nl80211_tx_power_setting)nla_get_u32(info->attrs[idx]);
...
static int nl80211_get_interface(struct sk_buff *skb, struct genl_info *info)
{
    struct sk_buff *msg;
    struct cfg80211_registered_device *rdev = (struct cfg80211_registered_device *)info->user_ptr[0];
    struct wireless_dev *wdev = (struct wireless_dev *)info->user_ptr[1];
                     ...
                     struct vif_params *params)
{
    ...
    if (info->attrs[NL80211_ATTR_MU_MIMO_GROUP_DATA]) {
        ...
        mumimo_groups =
            (const u8 *)nla_data(info->attrs[NL80211_ATTR_MU_MIMO_GROUP_DATA]);
    ...
    if (info->attrs[NL80211_ATTR_MU_MIMO_FOLLOW_MAC_ADDR]) {
        ...
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_MU_MIMO_FOLLOW_MAC_ADDR]);
        change = true;
    }
...
static int nl80211_set_interface(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IFTYPE]) {
        ntype = (enum nl80211_iftype)nla_get_u32(info->attrs[NL80211_ATTR_IFTYPE]);
        if (otype != ntype)
            change = true;
    }
...
static int nl80211_new_interface(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IFTYPE])
        type = (enum nl80211_iftype)nla_get_u32(info->attrs[NL80211_ATTR_IFTYPE]);
...
{
    struct nlattr *key;
    struct get_key_cookie *cookie = (struct get_key_cookie *)c;
...
static int nl80211_get_key(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        mac_addr = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_set_key(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (key.def) {
        ...
        if (info->attrs[NL80211_ATTR_MAC])
            mac_addr = (u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
                        ...
                        struct genl_info *info)
{
    ...
    acl_policy = (enum nl80211_acl_policy)nla_get_u32(info->attrs[NL80211_ATTR_ACL_POLICY]);
    ...
    acl = (struct cfg80211_acl_data *)kzalloc(struct_size(acl, mac_addrs, n_entries), GFP_KERNEL);
    if (!acl)
                     ...
                     struct cfg80211_bitrate_mask *mask)
{
    ...
    nla_for_each_nested(tx_rates, info->attrs[NL80211_ATTR_TX_RATES], rem) {
        enum nl80211_band band = (enum nl80211_band)nla_type(tx_rates);
        int err;
        ...
        if (tb[NL80211_TXRATE_GI]) {
            mask->control[band].gi =
                (enum nl80211_txrate_gi)nla_get_u8(tb[NL80211_TXRATE_GI]);
                ...
                struct cfg80211_beacon_data *bcn)
{
    ...
    if (attrs[NL80211_ATTR_BEACON_HEAD]) {
        bcn->head = (const u8 *)nla_data(attrs[NL80211_ATTR_BEACON_HEAD]);
    ...
    if (attrs[NL80211_ATTR_BEACON_TAIL]) {
        bcn->tail = (const u8 *)nla_data(attrs[NL80211_ATTR_BEACON_TAIL]);
    ...
    if (attrs[NL80211_ATTR_IE]) {
        bcn->beacon_ies = (const u8 *)nla_data(attrs[NL80211_ATTR_IE]);
        bcn->beacon_ies_len = nla_len(attrs[NL80211_ATTR_IE]);
    }

    if (attrs[NL80211_ATTR_IE_PROBE_RESP]) {
        bcn->proberesp_ies =
        (const u8 *)nla_data(attrs[NL80211_ATTR_IE_PROBE_RESP]);
    ...
    if (attrs[NL80211_ATTR_IE_ASSOC_RESP]) {
        bcn->assocresp_ies =
        (const u8 *)nla_data(attrs[NL80211_ATTR_IE_ASSOC_RESP]);
        bcn->assocresp_ies_len =
            nla_len(attrs[NL80211_ATTR_IE_ASSOC_RESP]);
    }

    if (attrs[NL80211_ATTR_PROBE_RESP]) {
        bcn->probe_resp = (const u8 *)nla_data(attrs[NL80211_ATTR_PROBE_RESP]);
    ...
    if (attrs[NL80211_ATTR_FTM_RESPONDER]) {
        ...
        if (tb[NL80211_FTM_RESP_ATTR_LCI]) {
            bcn->lci = (const u8 *)nla_data(tb[NL80211_FTM_RESP_ATTR_LCI]);
            bcn->lci_len = nla_len(tb[NL80211_FTM_RESP_ATTR_LCI]);
        }

        if (tb[NL80211_FTM_RESP_ATTR_CIVICLOC]) {
            bcn->civicloc = (const u8 *)nla_data(tb[NL80211_FTM_RESP_ATTR_CIVICLOC]);
...
static void nl80211_calculate_ap_params(struct cfg80211_ap_settings *params)
{
    ...
    if (cap && cap[1] >= sizeof(*params->ht_cap))
        params->ht_cap = (const struct ieee80211_ht_cap *)(cap + 2);
    cap = cfg80211_find_ie(WLAN_EID_VHT_CAPABILITY, ies, ies_len);
    if (cap && cap[1] >= sizeof(*params->vht_cap))
        params->vht_cap = (const struct ieee80211_vht_cap *)(cap + 2);
    cap = cfg80211_find_ext_ie(WLAN_EID_EXT_HE_CAPABILITY, ies, ies_len);
    if (cap && cap[1] >= sizeof(*params->he_cap) + 1)
        params->he_cap = (const struct ieee80211_he_cap_elem *)(cap + 3);
}
...
static int nl80211_start_ap(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_SSID]) {
        params.ssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_SSID]);
        params.ssid_len =
    ...
    if (info->attrs[NL80211_ATTR_HIDDEN_SSID])
        params.hidden_ssid = (enum nl80211_hidden_ssid)nla_get_u32(
            info->attrs[NL80211_ATTR_HIDDEN_SSID]);

    params.privacy = !!info->attrs[NL80211_ATTR_PRIVACY];

    if (info->attrs[NL80211_ATTR_AUTH_TYPE]) {
        params.auth_type = (enum nl80211_auth_type)nla_get_u32(
    ...
    if (info->attrs[NL80211_ATTR_SMPS_MODE]) {
        params.smps_mode =
            (enum nl80211_smps_mode)nla_get_u8(info->attrs[NL80211_ATTR_SMPS_MODE]);
                   ...
                   struct station_parameters *params)
{
    ...
    if (nla) {
        ...
        sta_flags = (struct nl80211_sta_flag_update *)nla_data(nla);
...
bool nl80211_put_sta_rate(struct sk_buff *msg, struct rate_info *info, int attr)
{
    ...
    case RATE_INFO_BW_20:
        rate_flg = (enum nl80211_rate_info)0;
        break;
                      ...
                      struct station_parameters *params)
{
    if (info->attrs[NL80211_ATTR_STA_SUPPORTED_CHANNELS]) {
        params->supported_channels =
             (const u8 *)nla_data(info->attrs[NL80211_ATTR_STA_SUPPORTED_CHANNELS]);
    ...
    if (info->attrs[NL80211_ATTR_STA_SUPPORTED_OPER_CLASSES]) {
        ...
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_STA_SUPPORTED_OPER_CLASSES]);
                    ...
                    struct station_parameters *params)
{
    ...
    if (info->attrs[NL80211_ATTR_HT_CAPABILITY])
        params->ht_capa =
            (const struct ieee80211_ht_cap *)nla_data(info->attrs[NL80211_ATTR_HT_CAPABILITY]);
    if (info->attrs[NL80211_ATTR_VHT_CAPABILITY])
        params->vht_capa =
            (const struct ieee80211_vht_cap *)nla_data(info->attrs[NL80211_ATTR_VHT_CAPABILITY]);
    if (info->attrs[NL80211_ATTR_HE_CAPABILITY]) {
        params->he_capa =
            (const struct ieee80211_he_cap_elem *)nla_data(info->attrs[NL80211_ATTR_HE_CAPABILITY]);
        params->he_capa_len =
                         ...
                         struct station_parameters *params)
{
    ...
    if (info->attrs[NL80211_ATTR_STA_TX_POWER_SETTING]) {
        ...
        params->txpwr.type = (enum nl80211_tx_power_setting)nla_get_u8(info->attrs[idx]);
...
static int nl80211_set_station(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_STA_SUPPORTED_RATES]) {
        params.supported_rates =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_STA_SUPPORTED_RATES]);
    ...
    if (info->attrs[NL80211_ATTR_STA_EXT_CAPABILITY]) {
        params.ext_capab =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_STA_EXT_CAPABILITY]);
    ...
    if (info->attrs[NL80211_ATTR_LOCAL_MESH_POWER_MODE])
        params.local_pm = (enum nl80211_mesh_power_mode)nla_get_u32(
...
static int nl80211_del_station(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        params.mac = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_get_mpath(struct sk_buff *skb, struct genl_info *info)
{
    ...
    dst = (u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_set_mpath(struct sk_buff *skb, struct genl_info *info)
{
    ...
    dst = (u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
    next_hop = (u8 *)nla_data(info->attrs[NL80211_ATTR_MPATH_NEXT_HOP]);
...
static int nl80211_set_bss(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_BSS_BASIC_RATES]) {
        params.basic_rates =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
...
static int nl80211_req_set_reg(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_USER_REG_HINT_TYPE])
        user_reg_hint_type =
          (enum nl80211_user_reg_hint_type)nla_get_u32(info->attrs[NL80211_ATTR_USER_REG_HINT_TYPE]);
    else
    ...
    case NL80211_USER_REG_HINT_CELL_BASE:
        ...
        data = (char *)nla_data(info->attrs[NL80211_ATTR_REG_ALPHA2]);
                     ...
                     struct mesh_setup *setup)
{
    ...
    if (tb[NL80211_MESH_SETUP_IE]) {
        ...
        setup->ie = (const u8 *)nla_data(ieattr);
                ...
                struct cfg80211_bss_selection *bss_select)
{
    ...
    nest = (struct nlattr *)nla_data(nla);
    ...
    if (attr[NL80211_BSS_SELECT_ATTR_BAND_PREF]) {
        ...
        bss_select->param.band_pref =
            (enum nl80211_band)nla_get_u32(attr[NL80211_BSS_SELECT_ATTR_BAND_PREF]);
    ...
    if (attr[NL80211_BSS_SELECT_ATTR_RSSI_ADJUST]) {
        ...
        adj_param = (struct nl80211_bss_select_rssi_adjust *)nla_data(attr[NL80211_BSS_SELECT_ATTR_RSSI_ADJUST]);
        bss_select->behaviour = NL80211_BSS_SELECT_ATTR_RSSI_ADJUST;
        bss_select->param.adjust.band = (enum nl80211_band)adj_param->band;
             ...
             bool is_sched_scan)
{
    ...
    if (is_sched_scan) {
        struct cfg80211_sched_scan_request *req = (struct cfg80211_sched_scan_request *)request;
        ...
        struct cfg80211_scan_request *req = (struct cfg80211_scan_request *)request;
...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (ie_len) {
        if (n_ssids)
            request->ie = (const u8 *)(request->ssids + n_ssids);
        else
    ...
    if (info->attrs[NL80211_ATTR_SCAN_SUPP_RATES]) {
        nla_for_each_nested(attr,
                    ...
                    tmp) {
            enum nl80211_band band = (enum nl80211_band)nla_type(attr);
             ...
             struct nlattr **attrs, int max_match_sets)
{
    ...
    request = (struct cfg80211_sched_scan_request *)kzalloc(sizeof(*request)
            + sizeof(*request->ssids) * n_ssids
    ...
    if (n_match_sets) {
        if (request->ie)
            request->match_sets = (struct cfg80211_match_set *)(request->ie + ie_len);
        else if (n_ssids)
            request->match_sets =
                (struct cfg80211_match_set *)(request->ssids + n_ssids);
        else
            request->match_sets =
                (struct cfg80211_match_set *)(request->channels + n_channels);
    }
    request->n_match_sets = n_match_sets;

    if (n_match_sets)
        request->scan_plans = (struct cfg80211_sched_scan_plan *)(request->match_sets +
                           n_match_sets);
    else if (request->ie)
        request->scan_plans = (struct cfg80211_sched_scan_plan *)(request->ie + ie_len);
    else if (n_ssids)
        request->scan_plans = (struct cfg80211_sched_scan_plan *)(request->ssids + n_ssids);
    else
        request->scan_plans = (struct cfg80211_sched_scan_plan *)(request->channels + n_channels);
    ...
    if (request->relative_rssi_set &&
        ...
        rssi_adjust = (struct nl80211_bss_select_rssi_adjust *)nla_data(
            attrs[NL80211_ATTR_SCHED_SCAN_RSSI_ADJUST]);
        request->rssi_adjust.band = (enum nl80211_band)rssi_adjust->band;
...
static int nl80211_channel_switch(struct sk_buff *skb, struct genl_info *info)
{
    ...
    params.counter_offsets_beacon =
        (const u16 *)nla_data(csa_attrs[NL80211_ATTR_CSA_C_OFF_BEACON]);
    ...
    if (csa_attrs[NL80211_ATTR_CSA_C_OFF_PRESP]) {
        ...
        params.counter_offsets_presp =
            (const u16 *)nla_data(csa_attrs[NL80211_ATTR_CSA_C_OFF_PRESP]);
...
static int nl80211_dump_survey(struct sk_buff *skb, struct netlink_callback *cb)
{
    ...
    attrbuf = (struct nlattr **)kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf), GFP_KERNEL);
    if (!attrbuf)
        return -ENOMEM;
...
static int nl80211_authenticate(struct sk_buff *skb, struct genl_info *info)
{
    ...
    ssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_SSID]);
    ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);

    if (info->attrs[NL80211_ATTR_IE]) {
        ie = (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]);
        ie_len = nla_len(info->attrs[NL80211_ATTR_IE]);
    }

    auth_type = (enum nl80211_auth_type)nla_get_u32(info->attrs[NL80211_ATTR_AUTH_TYPE]);
    ...
    if (info->attrs[NL80211_ATTR_AUTH_DATA]) {
        if (auth_type != NL80211_AUTHTYPE_SAE &&
            ...
            auth_type != NL80211_AUTHTYPE_FILS_PK)
            return -EINVAL;
        auth_data = (const u8 *)nla_data(info->attrs[NL80211_ATTR_AUTH_DATA]);
                   ...
                   int cipher_limit)
{
    ...
    if (info->attrs[NL80211_ATTR_PMK]) {
        ...
        if (!wiphy_ext_feature_isset(&rdev->wiphy,
            ...
            return -EINVAL;
        settings->psk = (const u8 *)nla_data(info->attrs[NL80211_ATTR_PMK]);
    }
    ...
    if (info->attrs[NL80211_ATTR_SAE_PASSWORD]) {
        if (!wiphy_ext_feature_isset(&rdev->wiphy,
            ...
            return -EINVAL;
        settings->sae_pwd =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_SAE_PASSWORD]);
...
static int nl80211_associate(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IE]) {
        req.ie = (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]);
        req.ie_len = nla_len(info->attrs[NL80211_ATTR_IE]);
    }

    if (info->attrs[NL80211_ATTR_USE_MFP]) {
        enum nl80211_mfp mfp =
            (enum nl80211_mfp)nla_get_u32(info->attrs[NL80211_ATTR_USE_MFP]);
    ...
    if (info->attrs[NL80211_ATTR_PREV_BSSID])
        req.prev_bssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_PREV_BSSID]);
    ...
    if (info->attrs[NL80211_ATTR_FILS_KEK]) {
        req.fils_kek = (const u8 *)nla_data(info->attrs[NL80211_ATTR_FILS_KEK]);
        req.fils_kek_len = nla_len(info->attrs[NL80211_ATTR_FILS_KEK]);
        if (!info->attrs[NL80211_ATTR_FILS_NONCES])
            return -EINVAL;
        req.fils_nonces =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_FILS_NONCES]);
    }
...
static int nl80211_join_ibss(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_MAC]) {
        ibss.bssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);

        if (!is_valid_ether_addr(ibss.bssid))
            return -EINVAL;
    }
    ibss.ssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_SSID]);
    ibss.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);

    if (info->attrs[NL80211_ATTR_IE]) {
        ibss.ie = (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]);
    ...
    if (info->attrs[NL80211_ATTR_BSS_BASIC_RATES]) {
        u8 *rates =
        (u8 *)nla_data(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        int n_rates =
            nla_len(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        struct ieee80211_supported_band *sband =
...
{
    struct cfg80211_registered_device *rdev = ((struct cfg80211_registered_device **)skb->cb)[0];
    void *hdr = ((void **)skb->cb)[1];
    struct nlmsghdr *nlhdr = nlmsg_hdr(skb);
    struct nlattr *data = ((struct nlattr **)skb->cb)[2];
...
static int nl80211_connect(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_AUTH_TYPE]) {
        connect.auth_type =
            (enum nl80211_auth_type)nla_get_u32(info->attrs[NL80211_ATTR_AUTH_TYPE]);
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        connect.bssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
    else if (info->attrs[NL80211_ATTR_MAC_HINT])
        connect.bssid_hint =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC_HINT]);
    connect.ssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_SSID]);
    connect.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);

    if (info->attrs[NL80211_ATTR_IE]) {
        connect.ie = (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]);
        connect.ie_len = nla_len(info->attrs[NL80211_ATTR_IE]);
    }

    if (info->attrs[NL80211_ATTR_USE_MFP]) {
        connect.mfp = (enum nl80211_mfp)nla_get_u32(info->attrs[NL80211_ATTR_USE_MFP]);
    ...
    if (info->attrs[NL80211_ATTR_PREV_BSSID])
        connect.prev_bssid =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_PREV_BSSID]);
    ...
    if (wiphy_ext_feature_isset(&rdev->wiphy,
        ...
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_FILS_ERP_USERNAME]);
        connect.fils_erp_username_len =
            nla_len(info->attrs[NL80211_ATTR_FILS_ERP_USERNAME]);
        connect.fils_erp_realm =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_FILS_ERP_REALM]);
        ...
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_FILS_ERP_RRK]);
                     ...
                     struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_AUTH_TYPE]) {
        ...
        connect.auth_type = (enum nl80211_auth_type)auth_type;
...
static int nl80211_setdel_pmksa(struct sk_buff *skb, struct genl_info *info)
{
    ...
    pmksa.pmkid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_PMKID]);

    if (info->attrs[NL80211_ATTR_MAC]) {
        pmksa.bssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
    } else if (info->attrs[NL80211_ATTR_SSID] &&
           info->attrs[NL80211_ATTR_FILS_CACHE_ID] &&
           (info->genlhdr->cmd == NL80211_CMD_DEL_PMKSA ||
            info->attrs[NL80211_ATTR_PMK])) {
        pmksa.ssid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_SSID]);
        pmksa.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);
        pmksa.cache_id =
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_FILS_CACHE_ID]);
    } else {
        return -EINVAL;
    }
    if (info->attrs[NL80211_ATTR_PMK]) {
        pmksa.pmk = (const u8 *)nla_data(info->attrs[NL80211_ATTR_PMK]);
...
static int nl80211_tdls_mgmt(struct sk_buff *skb, struct genl_info *info)
{
    ...
    peer = (u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_tdls_oper(struct sk_buff *skb, struct genl_info *info)
{
    ...
    operation = (enum nl80211_tdls_operation)nla_get_u8(info->attrs[NL80211_ATTR_TDLS_OPERATION]);
    peer = (u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_tx_mgmt(struct sk_buff *skb, struct genl_info *info)
{
    ...
    params.buf = (const u8 *)nla_data(info->attrs[NL80211_ATTR_FRAME]);
    ...
    if (info->attrs[NL80211_ATTR_CSA_C_OFFSETS_TX]) {
        ...
        params.csa_offsets =
            (const u16 *)nla_data(info->attrs[NL80211_ATTR_CSA_C_OFFSETS_TX]);
                ...
                u32 hysteresis)
{
    ...
    if (n_thresholds) {
        ...
        cqm_config = (struct cfg80211_cqm_config *)kzalloc(sizeof(struct cfg80211_cqm_config) +
...
static int nl80211_set_cqm(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (attrs[NL80211_ATTR_CQM_RSSI_THOLD] &&
        ...
        const s32 *thresholds =
            (const s32 *)nla_data(attrs[NL80211_ATTR_CQM_RSSI_THOLD]);
...
static int nl80211_join_mesh(struct sk_buff *skb, struct genl_info *info)
{
    ...
    setup.mesh_id = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MESH_ID]);
    ...
    if (info->attrs[NL80211_ATTR_BSS_BASIC_RATES]) {
        u8 *rates = (u8 *)nla_data(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        int n_rates =
            nla_len(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        struct ieee80211_supported_band *sband;
                    ...
                    struct cfg80211_wowlan *trig)
{
    ...
    if (tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_TOKEN]) {
        ...
        tok = (struct nl80211_wowlan_tcp_data_token *)nla_data(tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_TOKEN]);
    ...
    if (tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_SEQ]) {
        seq = (struct nl80211_wowlan_tcp_data_seq *)nla_data(tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_SEQ]);
    ...
    cfg = (struct cfg80211_wowlan_tcp *)kzalloc(size, GFP_KERNEL);
    if (!cfg)
        return -ENOMEM;
                   ...
                   struct cfg80211_wowlan *trig)
{
    ...
    tb = (struct nlattr **)kcalloc(NUM_NL80211_ATTR, sizeof(*tb), GFP_KERNEL);
    if (!tb)
        return -ENOMEM;
...
static int nl80211_set_wowlan(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]) {
        ...
        new_triggers.patterns = (struct cfg80211_pkt_pattern *)kcalloc(n_patterns,
        ...
        nla_for_each_nested(pat, tb[NL80211_WOWLAN_TRIG_PKT_PATTERN],
            ...
            mask_pat = (u8 *)kmalloc(mask_len + pat_len, GFP_KERNEL);
            if (!mask_pat) {
                err = -ENOMEM;
                goto error;
            }
    ...
    ntrig = (struct cfg80211_wowlan *)kmemdup(&new_triggers, sizeof(new_triggers), GFP_KERNEL);
    if (!ntrig) {
        err = -ENOMEM;
        goto error;
    }
                       ...
                       struct cfg80211_coalesce_rules *new_rule)
{
    ...
    if (tb[NL80211_ATTR_COALESCE_RULE_CONDITION])
        new_rule->condition =
            (enum nl80211_coalesce_condition)nla_get_u32(tb[NL80211_ATTR_COALESCE_RULE_CONDITION]);
    ...
    new_rule->patterns = (struct cfg80211_pkt_pattern *)kcalloc(n_patterns, sizeof(new_rule->patterns[0]),
                     GFP_KERNEL);
...
static int nl80211_set_coalesce(struct sk_buff *skb, struct genl_info *info)
{
    ...
    new_coalesce.rules = (struct cfg80211_coalesce_rules *)kcalloc(n_rules, sizeof(new_coalesce.rules[0]),
                     GFP_KERNEL);
    ...
    n_coalesce = (struct cfg80211_coalesce *)kmemdup(&new_coalesce, sizeof(new_coalesce), GFP_KERNEL);
    if (!n_coalesce) {
        err = -ENOMEM;
        goto error;
    }
...
static int nl80211_set_rekey_data(struct sk_buff *skb, struct genl_info *info)
{
    ...
    rekey_data.kek = (const u8 *)nla_data(tb[NL80211_REKEY_DATA_KEK]);
    rekey_data.kck = (const u8 *)nla_data(tb[NL80211_REKEY_DATA_KCK]);
    rekey_data.replay_ctr = (const u8 *)nla_data(tb[NL80211_REKEY_DATA_REPLAY_CTR]);
                ...
                struct genl_info *info)
{
    ...
    addr = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_register_beacons(struct sk_buff *skb, struct genl_info *info)
{
    ...
    nreg = (struct cfg80211_beacon_registration *)kzalloc(sizeof(*nreg), GFP_KERNEL);
    if (!nreg)
        return -ENOMEM;
                 ...
                 bool tx)
{
    ...
    filter = (struct cfg80211_nan_func_filter *)kcalloc(n_entries, sizeof(*func->rx_filters), GFP_KERNEL);
    if (!filter)
        return -ENOMEM;

    i = 0;
    nla_for_each_nested(attr, attr_filter, rem) {
        filter[i].filter = (const u8 *)nla_memdup(attr, GFP_KERNEL);
                ...
                struct genl_info *info)
{
    ...
    func = (struct cfg80211_nan_func *)kzalloc(sizeof(*func), GFP_KERNEL);
    if (!func)
        return -ENOMEM;
    ...
    func->type = (enum nl80211_nan_function_type)nla_get_u8(tb[NL80211_NAN_FUNC_TYPE]);
    ...
    if (tb[NL80211_NAN_FUNC_SERVICE_INFO]) {
        ...
        func->serv_spec_info =
            (const u8 *)kmemdup(nla_data(tb[NL80211_NAN_FUNC_SERVICE_INFO]),
                func->serv_spec_info_len,
                GFP_KERNEL);
    ...
    if (tb[NL80211_NAN_FUNC_SRF]) {
        ...
        if (srf_tb[NL80211_NAN_SRF_BF]) {
            ...
            func->srf_bf =
                (const u8 *)kmemdup(nla_data(srf_tb[NL80211_NAN_SRF_BF]),
            ...
            func->srf_macs =
                (struct mac_address *)kcalloc(n_entries, sizeof(*func->srf_macs),
                    GFP_KERNEL);
            if (!func->srf_macs) {
                err = -ENOMEM;
                goto out;
...
static int nl80211_update_ft_ies(struct sk_buff *skb, struct genl_info *info)
{
    ...
    ft_params.ie = (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]);
                       ...
                       struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_CRIT_PROT_ID])
        proto = (enum nl80211_crit_proto_id)nla_get_u16(info->attrs[NL80211_ATTR_CRIT_PROT_ID]);
                   ...
                   struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_QOS_MAP]) {
        pos = (u8 *)nla_data(info->attrs[NL80211_ATTR_QOS_MAP]);
        ...
        qos_map = (struct cfg80211_qos_map *)kzalloc(sizeof(struct cfg80211_qos_map), GFP_KERNEL);
        if (!qos_map)
            return -ENOMEM;
...
static int nl80211_add_tx_ts(struct sk_buff *skb, struct genl_info *info)
{
    ...
    peer = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_set_pmk(struct sk_buff *skb, struct genl_info *info)
{
    ...
    pmk_conf.aa = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
    if (memcmp(pmk_conf.aa, wdev->current_bss->pub.bssid, ETH_ALEN)) {
        ret = -EINVAL;
        goto out;
    }

    pmk_conf.pmk = (const u8 *)nla_data(info->attrs[NL80211_ATTR_PMK]);
    ...
    if (info->attrs[NL80211_ATTR_PMKR0_NAME]) {
        ...
        (const u8 *)nla_data(info->attrs[NL80211_ATTR_PMKR0_NAME]);
    }
...
static int nl80211_del_pmk(struct sk_buff *skb, struct genl_info *info)
{
    ...
    wdev_lock(wdev);
    aa = (const u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_external_auth(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_PMKID])
        params.pmkid = (const u8 *)nla_data(info->attrs[NL80211_ATTR_PMKID]);
...
static int nl80211_tx_control_port(struct sk_buff *skb, struct genl_info *info)
{
    ...
    buf = (const u8 *)nla_data(info->attrs[NL80211_ATTR_FRAME]);
    len = nla_len(info->attrs[NL80211_ATTR_FRAME]);
    dest = (u8 *)nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_update_owe_info(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IE]) {
        owe_info.ie = (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]);
...
{
    void **cb = (void **)msg->cb;
    struct cfg80211_registered_device *rdev = (struct cfg80211_registered_device *)cb[2];
