            ...
            bool signal_valid, unsigned long ts)
{
    ...
    if (found) {
         ...
         */
        _new = (struct cfg80211_internal_bss *)kzalloc(sizeof(*_new) + rdev->wiphy.bss_priv_size,
                  GFP_ATOMIC);
        if (!_new) {
            ies = (struct cfg80211_bss_ies *)rcu_dereference(tmp->pub.beacon_ies);
            if (ies)
                kfree_rcu(ies, rcu_head);
            ies = (struct cfg80211_bss_ies *)rcu_dereference(tmp->pub.proberesp_ies);
            if (ies)
                kfree_rcu(ies, rcu_head);
            goto drop;
        }
             ...
             enum nl80211_bss_scan_width scan_width)
{
    ...
    if (tmp && tmp[1] == 1) {
        ...
        if (tmp && tmp[1] >= sizeof(struct ieee80211_ht_operation)) {
            struct ieee80211_ht_operation *htop = (struct ieee80211_ht_operation *)(tmp + 2);
                ...
                gfp_t gfp)
{
     ...
     */
    ies = (struct cfg80211_bss_ies *)kzalloc(sizeof(*ies) + ielen, gfp);
    if (!ies)
        return NULL;
    ies->len = ielen;
    ies->tsf = tsf;
                   ...
                   const struct element *sub_elem)
{
    ...
    */
    ...
    next_sub = (const struct element *)&next_mbssid->data[1];
                  ...
                  u8 *merged_ie, size_t max_copy_len)
{
    ...
    while ((next_mbssid = cfg80211_get_profile_continuation(ie, ielen,
                                mbssid_elem,
                                sub_elem))) {
        const struct element *next_sub = (const struct element *)&next_mbssid->data[1];
                       ...
                       gfp_t gfp)
{
    ...
    new_ie = (u8 *)kmalloc(IEEE80211_MAX_DATA_LEN, gfp);
    if (!new_ie)
        return;

    profile = (u8 *)kmalloc(ielen, gfp);
    if (!profile)
        goto out;
                   ...
                   struct ieee80211_mgmt *mgmt, size_t len)
{
     ...
     * 2. skip MBSSID IE
     */
    new_ie = (u8 *)kzalloc(new_ie_len, GFP_ATOMIC);
    if (!new_ie)
        return;

    new_ies = (struct cfg80211_bss_ies *)kzalloc(sizeof(*new_ies) + new_ie_len, GFP_ATOMIC);
    if (!new_ies)
        goto out_free;
