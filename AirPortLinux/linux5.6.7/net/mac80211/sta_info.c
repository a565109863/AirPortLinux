                ...
                const u8 *addr, gfp_t gfp)
{
    ...
    sta = (struct sta_info *)kzalloc(sizeof(*sta) + hw->sta_data_size, gfp);
    if (!sta)
        return NULL;
    ...
    if (local->ops->wake_tx_queue) {
        ...
        for (i = 0; i < ARRAY_SIZE(sta->sta.txq); i++) {
            struct txq_info *txq = (struct txq_info *)((struct txq_info *)txq_data + i * size);
                     ...
                     struct sta_info *sta)
{
    ...
    for (state = IEEE80211_STA_NOTEXIST; state < sta->sta_state; state++) {
        err = drv_sta_state(local, sdata, sta, (enum ieee80211_sta_state)state, (enum ieee80211_sta_state)(state + 1));
        if (err)
            break;
    }
    ...
    for (; state > IEEE80211_STA_NOTEXIST; state--)
        WARN_ON(drv_sta_state(local, sdata, sta, (enum ieee80211_sta_state)state, (enum ieee80211_sta_state)(state - 1)));
...
static int sta_info_insert_finish(struct sta_info *sta) __acquires(RCU)
{
    ...
    sinfo = (struct station_info *)kzalloc(sizeof(struct station_info), GFP_KERNEL);
    if (!sinfo) {
        err = -ENOMEM;
        goto out_err;
    }
...
static void __sta_info_destroy_part2(struct sta_info *sta)
{
    ...
    while (sta->sta_state > IEEE80211_STA_NONE) {
        ret = sta_info_move_state(sta, (enum ieee80211_sta_state)(sta->sta_state - 1));
        if (ret) {
            WARN_ON_ONCE(1);
            break;
        }
    ...
    sinfo = (struct station_info *)kzalloc(sizeof(*sinfo), GFP_KERNEL);
    if (sinfo)
