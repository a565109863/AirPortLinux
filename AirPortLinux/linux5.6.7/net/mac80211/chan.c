            ...
            enum ieee80211_chanctx_mode mode)
{
    ...
    ctx = (struct ieee80211_chanctx *)kzalloc(sizeof(*ctx) + local->hw.chanctx_data_size, GFP_KERNEL);
    if (!ctx)
        return NULL;
              ...
              enum ieee80211_chanctx_mode mode)
{
    ...
    if (!ctx)
        return (struct ieee80211_chanctx *)ERR_PTR(-ENOMEM);

    err = ieee80211_add_chanctx(local, ctx);
    if (err) {
        kfree(ctx);
        return (struct ieee80211_chanctx *)ERR_PTR(err);
    }
                      ...
                      int n_vifs)
{
    ...
    vif_chsw = (struct ieee80211_vif_chanctx_switch *)kcalloc(n_vifs, sizeof(vif_chsw[0]), GFP_KERNEL);
    if (!vif_chsw)
        return -ENOMEM;
