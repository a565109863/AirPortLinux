                ...
                const struct element *check_inherit)
{
    ...
    for_each_element(elem, start, len) {
        ...
        case WLAN_EID_LINK_ID:
            ...
            elems->lnk_id = (const struct ieee80211_tdls_lnkie *)(pos - 2);
            break;
        case WLAN_EID_CHAN_SWITCH_TIMING:
        ...
        case WLAN_EID_HT_OPERATION:
            if (elen >= sizeof(struct ieee80211_ht_operation))
                elems->ht_operation = (const struct ieee80211_ht_operation *)pos;
            else
                elem_parse_failed = true;
            break;
        case WLAN_EID_VHT_CAPABILITY:
        ...
        case WLAN_EID_EXTENSION:
            if (pos[0] == WLAN_EID_EXT_HE_MU_EDCA &&
                ...
                elems->mu_edca_param_set = (const struct ieee80211_mu_edca_param_set *)&pos[1];
                if (calc_crc)
                    crc = crc32_be(crc, pos - 2, elen + 2);
            } else if (pos[0] == WLAN_EID_EXT_HE_CAPABILITY) {
                elems->he_cap = (const u8 *)&pos[1];
                elems->he_cap_len = elen - 1;
            } else if (pos[0] == WLAN_EID_EXT_HE_OPERATION &&
                   elen >= sizeof(*elems->he_operation) &&
                   elen >= ieee80211_he_oper_size(&pos[1])) {
                elems->he_operation = (const struct ieee80211_he_operation *)&pos[1];
            } else if (pos[0] == WLAN_EID_EXT_UORA && elen >= 1) {
                elems->uora_element = (const u8 *)&pos[1];
            } else if (pos[0] ==
                ...
                elems->max_channel_switch_time = pos + 1;
                   ...
                   elen == 3) {
                elems->mbssid_config_ie = (const struct ieee80211_multiple_bssid_configuration *)&pos[1];
            } else if (pos[0] == WLAN_EID_EXT_HE_SPR &&
                   elen >= sizeof(*elems->he_spr) &&
                   elen >= ieee80211_he_spr_size(&pos[1])) {
                elems->he_spr = (const struct ieee80211_he_spr *)&pos[1];
            }
            break;
        default:
            break;
...
bool ieee80211_chandef_vht_oper(struct ieee80211_hw *hw,
                ...
                struct cfg80211_chan_def *chandef)
{
    struct cfg80211_chan_def _new = *chandef;
    int cf0, cf1;
    ...
    case IEEE80211_VHT_CHANWIDTH_80MHZ:
        _new.width = NL80211_CHAN_WIDTH_80;
        _new.center_freq1 = cf0;
        ...
        if (ccf1) {
            ...
            if ((diff == 8) && support_160) {
                _new.width = NL80211_CHAN_WIDTH_160;
                _new.center_freq1 = cf1;
            } else if ((diff > 8) && support_80_80) {
                _new.width = NL80211_CHAN_WIDTH_80P80;
                _new.center_freq2 = cf1;
            }
        }
        break;
    ...
    if (!cfg80211_chandef_valid(&_new))
        return false;
