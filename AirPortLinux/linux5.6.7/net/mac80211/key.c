            ...
            const struct ieee80211_cipher_scheme *cs)
{
    ...
    if (WARN_ON(idx < 0 || idx >= NUM_DEFAULT_KEYS + NUM_DEFAULT_MGMT_KEYS))
        return (struct ieee80211_key *)ERR_PTR(-EINVAL);

    key = (struct ieee80211_key *)kzalloc(sizeof(struct ieee80211_key) + key_len, GFP_KERNEL);
    if (!key)
        return (struct ieee80211_key *)ERR_PTR(-ENOMEM);
    ...
    case WLAN_CIPHER_SUITE_CCMP:
        ...
        if (IS_ERR(key->u.ccmp.tfm)) {
            ...
            kfree(key);
            return (struct ieee80211_key *)ERR_PTR(err);
        }
        break;
    case WLAN_CIPHER_SUITE_CCMP_256:
            ...
            struct ieee80211_key_conf *keyconf)
{
    ...
    if (WARN_ON(!local->wowlan))
        return (struct ieee80211_key_conf *)ERR_PTR(-EINVAL);
    ...
    key = ieee80211_key_alloc(keyconf->cipher, keyconf->keyidx,
                  ...
                  0, NULL, NULL);
    if (IS_ERR(key))
        return (struct ieee80211_key_conf *)ERR_CAST(key);
    ...
    if (err)
        return (struct ieee80211_key_conf *)ERR_PTR(err);
