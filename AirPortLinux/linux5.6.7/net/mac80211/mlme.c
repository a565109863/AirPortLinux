                ...
                enum ieee80211_smps_mode smps)
{
    ...
    /* reserve and fill IE */
    pos = (typeof(pos))skb_put(skb, sizeof(struct ieee80211_ht_cap) + 2);
                 ...
                 struct ieee80211_vht_cap *ap_vht_cap)
{
    ...
    /* reserve and fill IE */
    pos = (typeof(pos))skb_put(skb, sizeof(struct ieee80211_vht_cap) + 2);
                ...
                struct ieee80211_supported_band *sband)
{
    ...
    pos = (typeof(pos))skb_put(skb, he_cap_size);
...
static void ieee80211_send_assoc(struct ieee80211_sub_if_data *sdata)
{
    ...
    if (assoc_data->ie_len)
        ext_capa = (typeof(ext_capa))cfg80211_find_elem(WLAN_EID_EXT_CAPABILITY,
                              assoc_data->ie,
                              assoc_data->ie_len);
    ...
    mgmt = (typeof(mgmt))skb_put_zero(skb, 24);
    ...
    /* SSID */
    pos = (typeof(pos))skb_put(skb, 2 + assoc_data->ssid_len);
    ie_start = pos;
    *pos++ = WLAN_EID_SSID;
    *pos++ = assoc_data->ssid_len;
    ...
    pos = (typeof(pos))skb_put(skb, supp_rates_len + 2);
    ...
    *pos++ = supp_rates_len;
    ...
    if (rates_len > count) {
        pos = (typeof(pos))skb_put(skb, rates_len - count + 2);
...
skip_rates:
    ...
    if (capab & WLAN_CAPABILITY_SPECTRUM_MGMT &&
        ...
        pos = (typeof(pos))skb_put(skb, 2 * sband->n_channels + 2);
    ...
    if (assoc_data->ie_len) {
        ...
        noffset = ieee80211_ie_split(assoc_data->ie, assoc_data->ie_len,
                         ...
                         offset);
        pos = (typeof(pos))skb_put(skb, noffset - offset);
    ...
    if (assoc_data->wmm) {
        ...
        pos = ieee80211_add_wmm_info_ie((u8 *)skb_put(skb, 9), qos_info);
    }
    ...
    ifmgd->assoc_req_ies = (typeof(ifmgd->assoc_req_ies))kmemdup(ie_start, pos - ie_start, GFP_ATOMIC);
                      ...
                      struct ieee80211_sub_if_data *sdata)
{
    ...
    nullfunc = (typeof(nullfunc))skb_put_zero(skb, 30);
                 ...
                 int *chan_pwr, int *pwr_reduction)
{
    ...
    triplet = (typeof(triplet))(country_ie + 3);
    country_ie_len -= 3;
...
__ieee80211_sta_handle_tspec_ac_params(struct ieee80211_sub_if_data *sdata)
{
    ...
    for (ac = 0; ac < IEEE80211_NUM_ACS; ac++) {
        ...
        if (tx_tspec->action == ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_NONE &&
            ...
            if (tx_tspec->downgraded)
                tx_tspec->action =
                    ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_STOP_DOWNGRADE;
        }

        switch (tx_tspec->action) {
        case ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_STOP_DOWNGRADE:
            /* take the original parameters */
            if (drv_conf_tx(local, sdata, ac, &sdata->tx_conf[ac]))
                sdata_err(sdata,
                      "failed to set TX queue parameters for queue %d\n",
                      ac);
            tx_tspec->action = ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_NONE;
            tx_tspec->downgraded = false;
            ret = true;
            break;
        case ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_DOWNGRADE:
            ...
            break;
        case ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_NONE:
            /* nothing now */
            break;
        }
    }
...
{
    struct ieee80211_bss *bss = (typeof(bss))cbss->priv;
    struct ieee80211_local *local = sdata->local;
    struct ieee80211_bss_conf *bss_conf = &sdata->vif.bss_conf;
                       ...
                       u16 tx_time)
{
    ...
    if (tx_tspec->consumed_tx_time >= tx_tspec->admitted_time) {
        ...
        tx_tspec->action = ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_DOWNGRADE;
        schedule_delayed_work(&ifmgd->tx_tspec_wk, 0);
    }
}
                    ...
                    struct ieee802_11_elems *elems)
{
    ...
    if (!is_6ghz &&
        ...
        ies = rcu_dereference(cbss->ies);
        if (ies)
            bss_ies = (typeof(bss_ies))kmemdup(ies, sizeof(*ies) + ies->len,
                      GFP_ATOMIC);
        rcu_read_unlock();
        if (!bss_ies)
            return false;
...
static void ieee80211_rx_mgmt_probe_resp(struct ieee80211_sub_if_data *sdata,
                     struct sk_buff *skb)
{
    struct ieee80211_mgmt *mgmt = (typeof(mgmt))skb->data;
    struct ieee80211_if_managed *ifmgd;
    struct ieee80211_rx_status *rx_status = (typeof(rx_status)) skb->cb;
    struct ieee80211_channel *channel;
                     ...
                     struct ieee80211_rx_status *rx_status)
{
    ...
    if (ieee80211_is_s1g_beacon(mgmt->frame_control)) {
        struct ieee80211_ext *ext = (typeof(ext)) mgmt;
    ...
    if (!ieee80211_is_s1g_beacon(hdr->frame_control))
        ncrc = crc32_be(0, (unsigned char const *)&mgmt->u.beacon.beacon_int, 4);
    ...
    if (sta && elems.opmode_notif)
        ieee80211_vht_handle_opmode(sdata, sta, *elems.opmode_notif,
                        (enum nl80211_band)rx_status->band);
    mutex_unlock(&local->sta_mtx);
                  ...
                  struct sk_buff *skb)
{
    ...
    case IEEE80211_STYPE_BEACON:
        ieee80211_rx_mgmt_beacon(sdata, (struct ieee80211_hdr *)mgmt,
                     skb->len, rx_status);
        break;
                     ...
                     struct cfg80211_bss *cbss)
{
    ...
    if (ht_cap_ie && ht_cap_ie[1] >= sizeof(*ht_cap)) {
        ht_cap = (typeof(ht_cap))(ht_cap_ie + 2);
    ...
    if (vht_cap_ie && vht_cap_ie[1] >= sizeof(*vht_cap)) {
        ...
        vht_cap = (typeof(vht_cap))(vht_cap_ie + 2);
                  ...
                  struct cfg80211_bss *cbss)
{
    ...
    if (!(ifmgd->flags & IEEE80211_STA_DISABLE_HT) && !is_6ghz) {
        ...
        if (ht_oper_ie && ht_oper_ie[1] >= sizeof(*ht_oper))
            ht_oper = (typeof(ht_oper))(ht_oper_ie + 2);
    ...
    if (!(ifmgd->flags & IEEE80211_STA_DISABLE_VHT) && !is_6ghz) {
        ...
        if (vht_oper_ie && vht_oper_ie[1] >= sizeof(*vht_oper))
            vht_oper = (typeof(vht_oper))(vht_oper_ie + 2);
        if (vht_oper && !ht_oper) {
            vht_oper = NULL;
            sdata_info(sdata,
    ...
    if (!(ifmgd->flags & IEEE80211_STA_DISABLE_HE)) {
        ...
        if (he_oper_ie &&
            ...
            he_oper = (typeof(he_oper))(he_oper_ie + 3);
        else
            he_oper = NULL;
    ...
    if (sband->band == NL80211_BAND_S1GHZ) {
        ...
        if (s1g_oper_ie && s1g_oper_ie[1] >= sizeof(*s1g_oper))
            s1g_oper = (typeof(s1g_oper))(s1g_oper_ie + 2);
        else
            sdata_info(sdata,
               ...
               struct cfg80211_auth_request *req)
{
    ...
    auth_data = (typeof(auth_data))kzalloc(sizeof(*auth_data) + req->auth_data_len +
            ...
            struct cfg80211_assoc_request *req)
{
    ...
    struct ieee80211_local *local = sdata->local;
    struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
    struct ieee80211_bss *bss = (typeof(bss))req->bss->priv;
    ...
    assoc_data = (typeof(assoc_data))kzalloc(sizeof(*assoc_data) + req->ie_len, GFP_KERNEL);
    if (!assoc_data)
        return -ENOMEM;
    ...
    if (req->use_mfp) {
        ifmgd->mfp = ieee80211_if_managed::IEEE80211_MFP_REQUIRED;
        ifmgd->flags |= IEEE80211_STA_MFP_ENABLED;
    } else {
        ifmgd->mfp = ieee80211_if_managed::IEEE80211_MFP_DISABLED;
        ifmgd->flags &= ~IEEE80211_STA_MFP_ENABLED;
    }
