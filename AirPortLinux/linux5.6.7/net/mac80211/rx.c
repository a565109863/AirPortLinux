                     ...
                     unsigned int rtap_space)
{
    ...
    hdr = (typeof(hdr))(skb->data + rtap_space);
                 ...
                 struct sk_buff *skb)
{
    ...
    if (status->flag & RX_FLAG_RADIOTAP_VENDOR_DATA) {
        ...
        rtap = (typeof(rtap))&skb->data[vendor_data_offset];
                 ...
                 int rtap_len, bool has_fcs)
{
    ...
    rthdr = (typeof(rthdr))skb_push(skb, rtap_len);
             ...
             struct ieee80211_rate *rate)
{
    ...
    if (unlikely(status->flag & RX_FLAG_RADIOTAP_VENDOR_DATA)) {
        struct ieee80211_vendor_radiotap *rtap =
            (typeof(rtap))(origskb->data + rtap_space);
...
{
    struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)rx->skb->data;
    char *dev_addr = (typeof(dev_addr))rx->sdata->vif.addr;
    ...
    if (ieee80211_is_data(hdr->frame_control)) {
        if (is_multicast_ether_addr(hdr->addr1)) {
            ...
            if (ether_addr_equal(hdr->addr3, (const u8 *)dev_addr))
                return RX_DROP_MONITOR;
        } else {
            if (!ieee80211_has_a4(hdr->frame_control))
                return RX_DROP_MONITOR;
            if (ether_addr_equal(hdr->addr4, (const u8 *)dev_addr))
                return RX_DROP_MONITOR;
        }
    }
...
ieee80211_rx_h_uapsd_and_pspoll(struct ieee80211_rx_data *rx)
{
    ...
    struct ieee80211_hdr *hdr = (typeof(hdr))rx->skb->data;
    struct ieee80211_rx_status *status = IEEE80211_SKB_RXCB(rx->skb);
...
ieee80211_rx_h_data(struct ieee80211_rx_data *rx)
{
    ...
    if (unlikely(((struct ethhdr *)rx->skb->data)->h_proto ==
        ...
        struct ieee80211_tdls_data *tf = (typeof(tf))rx->skb->data;
                       ...
                       size_t len)
{
    ...
    resp = (typeof(resp))skb_put_zero(skb, 24);
...
ieee80211_rx_h_action_return(struct ieee80211_rx_data *rx)
{
    ...
    nskb = skb_copy_expand(rx->skb, local->hw.extra_tx_headroom, 0,
                   GFP_ATOMIC);
    if (nskb) {
        struct ieee80211_mgmt *nmgmt = (typeof(nmgmt))nskb->data;
...
{
    struct ieee80211_sub_if_data *sdata = rx->sdata;
    struct ieee80211_mgmt *mgmt = (typeof(mgmt))rx->skb->data;
    __le16 stype;
...
void ieee80211_check_fast_rx(struct sta_info *sta)
{
 ...
 clear:
    ...
    if (assign)
        _new = (typeof(_new))kmemdup(&fastrx, sizeof(fastrx), GFP_KERNEL);

    spin_lock_bh(&sta->lock);
    old = rcu_dereference_protected(sta->fast_rx, true);
    rcu_assign_pointer(sta->fast_rx, _new);
...
#define FAST_RX_CRYPT_FLAGS    (RX_FLAG_PN_VALIDATED | RX_FLAG_DECRYPTED)
    ...
    if (!(status->rx_flags & IEEE80211_RX_AMSDU)) {
        ...
        payload = (typeof(payload))(skb->data + snap_offs);
