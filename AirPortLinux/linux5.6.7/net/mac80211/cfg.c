...
static struct wireless_dev *ieee80211_add_iface(struct wiphy *wiphy,
                        ...
                        struct vif_params *params)
{
    struct ieee80211_local *local = (struct ieee80211_local *)wiphy_priv(wiphy);
                    ...
                    const struct ieee80211_csa_settings *csa)
{
    ...
    _new = (struct probe_resp *)kzalloc(sizeof(struct probe_resp) + resp_len, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;
    ...
    if (csa)
        memcpy(_new->csa_counter_offsets, csa->counter_offsets_presp,
               csa->n_counter_offsets_presp *
               sizeof(_new->csa_counter_offsets[0]));
                ...
                const u8 *civicloc, size_t civicloc_len)
{
    ...
    _new = (struct ieee80211_ftm_responder_params *)kzalloc(sizeof(*_new) + len, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;
                   ...
                   const struct ieee80211_csa_settings *csa)
{
    ...
    _new = (struct beacon_data *)kzalloc(size, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;
    ...
    if (csa) {
        _new->csa_current_counter = csa->count;
        memcpy(_new->csa_counter_offsets, csa->counter_offsets_beacon,
...
cfg80211_beacon_dup(struct cfg80211_beacon_data *beacon)
{
    ...
    new_beacon = (struct cfg80211_beacon_data *)kzalloc(sizeof(*new_beacon) + len, GFP_KERNEL);
    if (!new_beacon)
        return NULL;
                  ...
                  const u8 *peer, u64 *cookie)
{
    ...
    nullfunc = (struct ieee80211_qos_hdr *)skb_put(skb, size);
                 ...
                 struct cfg80211_qos_map *qos_map)
{
    ...
    if (qos_map) {
        new_qos_map = (struct mac80211_qos_map *)kzalloc(sizeof(*new_qos_map), GFP_KERNEL);
        if (!new_qos_map)
            return -ENOMEM;
