			       ...
			       int scale_index, int attempts, int successes)
{
	...
	if (tbl->column != RS_COLUMN_INVALID) {
        #define lq_sta_pers iwl_lq_sta::lq_sta_pers
		struct lq_sta_pers *pers = (struct lq_sta_pers *)&mvmsta->lq_sta.rs_drv.pers;
			    ...
			    struct ieee80211_tx_rate_control *txrc)
{
	...
	lq_sta = (struct iwl_lq_sta *)mvm_sta;
	iwl_mvm_hwrate_to_tx_rate(lq_sta->last_rate_n_flags,
				  (enum nl80211_band)info->band, &info->control.rates[0]);
	...
	if (lq_sta->rs_state != RS_STATE_STAY_IN_COLUMN) {
		...
		last_ucode_rate = ucode_rate_from_rs_rate(mvm,
							  optimal_rate);
		iwl_mvm_hwrate_to_tx_rate(last_ucode_rate, (enum nl80211_band)info->band,
					  &txrc->reported_rate);
	}
}
				   ...
				   bool ndp)
{
	...
	if (time_after(jiffies,
		 ...
		 */
		rs_drv_rate_init(mvm, sta, (enum nl80211_band)info->band);
		return;
	}
	lq_sta->last_tx = jiffies;
...
{
	struct iwl_op_mode *op_mode __maybe_unused = (struct iwl_op_mode *)mvm_r;
	struct iwl_mvm *mvm __maybe_unused = IWL_OP_MODE_GET_MVM(op_mode);
			...
			const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(buf, (void *)user_buf, buf_size))
		return -EFAULT;
