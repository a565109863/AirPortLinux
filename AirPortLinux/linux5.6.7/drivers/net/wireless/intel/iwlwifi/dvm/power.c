				...
				struct iwl_powertable_cmd *cmd)
{
	...
	if (priv->wowlan)
		...
		iwl_static_sleep_cmd(priv, cmd,
		    (enum iwl_power_level)iwl_tt_current_power_mode(priv), dtimper);
	} else if (!enabled)
		iwl_power_sleep_cam_cmd(priv, cmd);
	else if (priv->power_data.debug_sleep_level_override >= 0)
		iwl_static_sleep_cmd(priv, cmd,
				     (enum iwl_power_level)priv->power_data.debug_sleep_level_override,
				     dtimper);
	else {
		...
		if (iwlwifi_mod_params.power_level > IWL_POWER_INDEX_1 &&
			...
			iwl_static_sleep_cmd(priv, cmd,
				(enum iwl_power_level)(iwlwifi_mod_params.power_level - 1), dtimper);
		else
			iwl_static_sleep_cmd(priv, cmd,
