...
{
    struct crypto_instance *inst = (struct crypto_instance *)alg;
...
free_larval:
    kfree(larval);
err:
    larval = (struct crypto_larval *)ERR_PTR(ret);
    goto out;
}
...
found:
    ...
    list_for_each_entry(q, &crypto_alg_list, cra_list) {
        ...
        if (crypto_is_larval(q)) {
            struct crypto_larval *larval = (struct crypto_larval *)q;
            ...
            if (best && crypto_mod_get(alg))
                ...
                larval->adult = (struct crypto_alg *)ERR_PTR(-EAGAIN);
                 ...
                 struct crypto_instance *inst)
{
    ...
    larval = (struct crypto_larval *)ERR_PTR(-EAGAIN);
...
static struct crypto_alg *crypto_spawn_alg(struct crypto_spawn *spawn)
{
    ...
    return alg ?: (struct crypto_alg *)ERR_PTR(-EAGAIN);
}
                    ...
                    u32 mask)
{
    ...
    if (IS_ERR(alg))
        return (struct crypto_tfm *)ERR_CAST(alg);

    tfm = (struct crypto_tfm *)ERR_PTR(-EINVAL);
...
struct crypto_attr_type *crypto_get_attr_type(struct rtattr **tb)
{
    ...
    if (!rta)
        return (struct crypto_attr_type *)ERR_PTR(-ENOENT);
//    if (RTA_PAYLOAD(rta) < sizeof(*algt))
//        return (struct crypto_attr_type *)ERR_PTR(-EINVAL);
...
const char *crypto_attr_alg_name(struct rtattr *rta)
{
    ...
    if (!rta)
        return (const char *)ERR_PTR(-ENOENT);
//    if (RTA_PAYLOAD(rta) < sizeof(*alga))
//        return (const char *)ERR_PTR(-EINVAL);
