...
//
    ...
    data = (u8 *)kmalloc_reserve(size, gfp_mask, node, &pfmemalloc);
    if (!data)
        goto nodata;
...
struct sk_buff *__build_skb(void *data, unsigned int frag_size)
{
    ...
    skb = (struct sk_buff *)kmalloc(sizeof(struct sk_buff), GFP_ATOMIC);
    if (unlikely(!skb))
        return NULL;
...
//        n = kmem_cache_alloc(skbuff_head_cache, gfp_mask);
        n = (struct sk_buff *)kmalloc(sizeof(*n), gfp_mask);
        if (!n)
            return NULL;
             ...
             gfp_t gfp_mask)
{
    ...
    if (skb_pfmemalloc(skb))
        gfp_mask |= __GFP_MEMALLOC;
    data = (u8 *)kmalloc_reserve(size + SKB_DATA_ALIGN(sizeof(struct skb_shared_info)),
...
//     */
//    if (__splice_segment(virt_to_page(skb->data, len),
...
//                          copy, p, p_off, p_len, copied) {
//                vaddr = kmap_atomic(p);
//                memcpy(vaddr + p_off, (char *)from + copied, p_len);
//                kunmap_atomic(vaddr);
//            }
...
static struct sk_buff *skb_reorder_vlan_header(struct sk_buff *skb)
{
    ...
    if (meta_len) {
        meta = (char *)skb_metadata_end(skb) - meta_len;
        memmove((char *)meta + VLAN_HLEN, meta, meta_len);
    }
