...
{
    return (const u8 *)ieee80211_bss_get_elem(bss, id);
}
...
struct wiphy {
    ...
    char priv[NETDEV_ALIGN] __aligned(NETDEV_ALIGN);
};
...
{
    BUG_ON(!priv);
    return (struct wiphy *)container_of(priv, struct wiphy, priv);
}
               ...
               unsigned int match_offset)
{
    ...
    return (const u8 *)cfg80211_find_elem_match(eid, ies, len,
                        match, match_len,
                        match_offset ?
                            match_offset - 2 : 0);
}
...
{
    return (const u8 *)cfg80211_find_vendor_elem(oui, oui_type, ies, len);
}
