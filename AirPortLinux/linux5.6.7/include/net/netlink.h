                   ...
                   struct netlink_ext_ack *extack)
{
    ...
    return __nla_parse(tb, maxtype, (const struct nlattr *)nla_data(nla), nla_len(nla), policy,
               NL_VALIDATE_STRICT, extack);
}
...
static inline int __nla_validate_nested(const struct nlattr *start, int maxtype,
                    ...
                    struct netlink_ext_ack *extack)
{
    return __nla_validate((const struct nlattr *)nla_data(start), nla_len(start), maxtype, policy,
                  validate, extack);
}
...
#define nla_for_each_nested(pos, nla, rem) \
    nla_for_each_attr(pos, (typeof(pos))nla_data(nla), nla_len(nla), rem)
