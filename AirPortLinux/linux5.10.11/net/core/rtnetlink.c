...
bool refcount_dec_and_mutex_lock(refcount_t *r, struct mutex *m)
{
    return 0;
}
                  ...
                  unsigned int flags)
{
    ...
    if (tab == NULL) {
        tab = (typeof tab)kcalloc(RTM_NR_MSGTYPES, sizeof(void *), GFP_KERNEL);
        if (!tab)
            goto unlock;
    ...
    if (old) {
        link = (typeof link)kmemdup(old, sizeof(*old), GFP_KERNEL);
        if (!link)
            goto unlock;
    } else {
        link = (typeof link)kzalloc(sizeof(*link), GFP_KERNEL);
        if (!link)
            goto unlock;
    }
...
struct net *rtnl_get_net_ns_capable(struct sock *sk, int netnsid)
{
    ...
    if (!net)
        return (typeof net)ERR_PTR(-EINVAL);
    ...
    if (!sk_ns_capable(sk, net->user_ns, CAP_NET_ADMIN)) {
        put_net(net);
        return (typeof net)ERR_PTR(-EACCES);
    }
    return net;
}
                         ...
                         struct nlattr *tb[], int cap)
{
    ...
    if (!netlink_ns_capable(skb, net->user_ns, cap)) {
        put_net(net);
        return (typeof net)ERR_PTR(-EPERM);
    }
...
static int do_setvfinfo(struct net_device *dev, struct nlattr **tb)
{
    ...
    if (tb[IFLA_VF_MAC]) {
        struct ifla_vf_mac *ivm = (typeof ivm)nla_data(tb[IFLA_VF_MAC]);
    ...
    if (tb[IFLA_VF_VLAN]) {
        struct ifla_vf_vlan *ivv = (typeof ivv)nla_data(tb[IFLA_VF_VLAN]);
    ...
    if (tb[IFLA_VF_VLAN_LIST]) {
        ...
        nla_for_each_nested(attr, tb[IFLA_VF_VLAN_LIST], rem) {
            ...
            if (len >= MAX_VLAN_LIST_LEN)
                return -EOPNOTSUPP;
            ivvl[len] = (struct ifla_vf_vlan_info *)nla_data(attr);
...
//        struct ifla_vf_tx_rate *ivt = (typeof ivt)nla_data(tb[IFLA_VF_TX_RATE]);
...
//            return err;
    ...
    if (tb[IFLA_VF_RATE]) {
        struct ifla_vf_rate *ivt = (typeof ivt)nla_data(tb[IFLA_VF_RATE]);
    ...
    if (tb[IFLA_VF_SPOOFCHK]) {
        struct ifla_vf_spoofchk *ivs = (typeof ivs)nla_data(tb[IFLA_VF_SPOOFCHK]);
    ...
    if (tb[IFLA_VF_LINK_STATE]) {
        struct ifla_vf_link_state *ivl = (typeof ivl)nla_data(tb[IFLA_VF_LINK_STATE]);
    ...
    if (tb[IFLA_VF_RSS_QUERY_EN]) {
        ...
        err = -EOPNOTSUPP;
        ivrssq_en = (typeof ivrssq_en)nla_data(tb[IFLA_VF_RSS_QUERY_EN]);
    ...
    if (tb[IFLA_VF_TRUST]) {
        struct ifla_vf_trust *ivt = (typeof ivt)nla_data(tb[IFLA_VF_TRUST]);
    ...
    if (tb[IFLA_VF_IB_NODE_GUID]) {
        struct ifla_vf_guid *ivt = (typeof ivt)nla_data(tb[IFLA_VF_IB_NODE_GUID]);
    ...
    if (tb[IFLA_VF_IB_PORT_GUID]) {
        struct ifla_vf_guid *ivt = (typeof ivt)nla_data(tb[IFLA_VF_IB_PORT_GUID]);
              ...
              struct nlattr **tb, char *ifname, int status)
{
    ...
    if (tb[IFLA_MAP]) {
        ...
        u_map = (typeof u_map)nla_data(tb[IFLA_MAP]);
    ...
    if (tb[IFLA_ADDRESS]) {
        ...
        len = sizeof(sa_family_t) + max_t(size_t, dev->addr_len,
                          sizeof(*sa));
        sa = (typeof sa)kmalloc(len, GFP_KERNEL);
        if (!sa) {
            err = -ENOMEM;
            goto errout;
        }
    ...
    if (tb[IFLA_IFALIAS]) {
        err = dev_set_alias(dev, (const char *)nla_data(tb[IFLA_IFALIAS]),
                    ...
                    struct netlink_ext_ack *extack)
{
    ...
    if (num_tx_queues < 1 || num_tx_queues > 4096) {
        ...
        return (typeof dev)ERR_PTR(-EINVAL);
    }
    ...
    if (!dev)
        return (typeof dev)ERR_PTR(-ENOMEM);
            ...
            struct netlink_ext_ack *extack)
{
    ...
    attr = (typeof attr)kmalloc_array(RTNL_MAX_TYPE + 1, sizeof(*attr), GFP_KERNEL);
    if (!attr)
        return -ENOMEM;
            ...
            struct netlink_ext_ack *extack)
{
    ...
    addr = (typeof addr)nla_data(tb[NDA_LLADDR]);
...
//                return -EINVAL;
//            }
//            *addr = (typeof *addr)nla_data(tb[i]);
//            break;
//        case NDA_VLAN:
