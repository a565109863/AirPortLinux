...
static struct wireless_dev *ieee80211_add_iface(struct wiphy *wiphy,
                        ...
                        struct vif_params *params)
{
    struct ieee80211_local *local = (typeof(local))wiphy_priv(wiphy);
    ...
    if (err)
        return (struct wireless_dev *)ERR_PTR(err);
                      ...
                      struct wireless_dev *wdev)
{
    ...
    ret = ieee80211_check_combinations(sdata, NULL, (enum ieee80211_chanctx_mode)0, 0);
                 ...
                 struct key_params *params)
{
    ...
    case NL80211_IFTYPE_STATION:
        if (sdata->u.mgd.mfp != ieee80211_if_managed::IEEE80211_MFP_DISABLED)
...
{
    struct ieee80211_local *local = (typeof(local))wdev_priv(dev->ieee80211_ptr);
                    ...
                    const struct ieee80211_csa_settings *csa)
{
    ...
    _new = (typeof(_new))kzalloc(sizeof(struct probe_resp) + resp_len, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;

    _new->len = resp_len;
    memcpy(_new->data, resp, resp_len);

    if (csa)
        memcpy(_new->cntdwn_counter_offsets, csa->counter_offsets_presp,
               csa->n_counter_offsets_presp *
               sizeof(_new->cntdwn_counter_offsets[0]));

    rcu_assign_pointer(sdata->u.ap.probe_resp, _new);
    if (old)
...
{
    struct fils_discovery_data *_new, *old = NULL;
    ...
    _new = (typeof(_new))kzalloc(sizeof(*_new) + params->tmpl_len, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;
    _new->len = params->tmpl_len;
    memcpy(_new->data, params->tmpl, params->tmpl_len);
    rcu_assign_pointer(sdata->u.ap.fils_discovery, _new);
...
{
    struct unsol_bcast_probe_resp_data *_new, *old = NULL;
    ...
    _new->len = params->tmpl_len;
    memcpy(_new->data, params->tmpl, params->tmpl_len);
    rcu_assign_pointer(sdata->u.ap.unsol_bcast_probe_resp, _new);
...
{
    struct ieee80211_ftm_responder_params *_new, *old;
    ...
    _new = (typeof(_new))kzalloc(sizeof(*_new) + len, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;

    pos = (u8 *)(_new + 1);
    if (lci_len) {
        _new->lci_len = lci_len;
        _new->lci = pos;
    ...
    if (civicloc_len) {
        _new->civicloc_len = civicloc_len;
        _new->civicloc = pos;
        memcpy(pos, civicloc, civicloc_len);
        pos += civicloc_len;
    }

    bss_conf->ftmr_params = _new;
    kfree(old);
                   ...
                   const struct ieee80211_csa_settings *csa)
{
    ...
    size = sizeof(*_new) + new_head_len + new_tail_len;

    _new = (typeof(_new))kzalloc(size, GFP_KERNEL);
    if (!_new)
        return -ENOMEM;
     ...
     */
    _new->head = ((u8 *) _new) + sizeof(*_new);
    _new->tail = _new->head + new_head_len;
    _new->head_len = new_head_len;
    _new->tail_len = new_tail_len;

    if (csa) {
        _new->cntdwn_current_counter = csa->count;
        memcpy(_new->cntdwn_counter_offsets, csa->counter_offsets_beacon,
    ...
    /* copy in head */
    if (params->head)
        memcpy(_new->head, params->head, new_head_len);
    else
        memcpy(_new->head, old->head, new_head_len);

    /* copy in optional tail */
    if (params->tail)
        memcpy(_new->tail, params->tail, new_tail_len);
    else
        if (old)
            memcpy(_new->tail, old->tail, new_tail_len);
    ...
    rcu_assign_pointer(sdata->u.ap.beacon, _new);
                ...
                struct station_parameters *params)
{
    ...
    if (params->he_capa)
        ieee80211_he_cap_ie_to_sta_he_cap(sdata, sband,
                          ...
                          params->he_capa_len,
                          (const struct ieee80211_he_6ghz_capa *)params->he_6ghz_capa,
                          sta);
...
{
    return __ieee80211_suspend((struct ieee80211_hw *)wiphy_priv(wiphy), wowlan);
}

static int ieee80211_resume(struct wiphy *wiphy)
{
    return __ieee80211_resume((struct ieee80211_hw *)wiphy_priv(wiphy));
}
#else
...
{
    ieee80211_scan_cancel((struct ieee80211_local *)wiphy_priv(wiphy));
}
...
cfg80211_beacon_dup(struct cfg80211_beacon_data *beacon)
{
    ...
    new_beacon = (typeof(new_beacon))kzalloc(sizeof(*new_beacon) + len, GFP_KERNEL);
    if (!new_beacon)
        return NULL;
                  ...
                  const u8 *peer, u64 *cookie)
{
    ...
    nullfunc = (typeof(nullfunc))skb_put(skb, size);
...
{
    drv_set_wakeup((struct ieee80211_local *)wiphy_priv(wiphy), enabled);
}
#endif
                 ...
                 struct cfg80211_qos_map *qos_map)
{
    ...
    if (qos_map) {
        new_qos_map = (typeof(new_qos_map))kzalloc(sizeof(*new_qos_map), GFP_KERNEL);
        if (!new_qos_map)
            return -ENOMEM;
                   ...
                   u8 tsid, const u8 *peer)
{
    ...
    for (ac = 0; ac < IEEE80211_NUM_ACS; ac++) {
         ...
         */
        tx_tspec->action = ieee80211_sta_tx_tspec::TX_TSPEC_ACTION_STOP_DOWNGRADE;
                   ...
                   gfp_t gfp)
{
    ...
    func = (typeof(func))idr_find(&sdata->u.nan.function_inst_ids, inst_id);
    if (WARN_ON(!func)) {
        spin_unlock_bh(&sdata->u.nan.func_lock);
        return;
    }
                  ...
                  gfp_t gfp)
{
    ...
    func = (typeof(func))idr_find(&sdata->u.nan.function_inst_ids,  match->inst_id);
    if (WARN_ON(!func)) {
        spin_unlock_bh(&sdata->u.nan.func_lock);
        return;
    }
