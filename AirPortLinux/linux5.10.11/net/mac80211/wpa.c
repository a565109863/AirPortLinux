...
ieee80211_tx_h_michael_mic_add(struct ieee80211_tx_data *tx)
{
    ...
    mic = (u8 *)skb_put(skb, MICHAEL_MIC_LEN);
...
static int tkip_encrypt_skb(struct ieee80211_tx_data *tx, struct sk_buff *skb)
{
    ...
    pos = (u8 *)skb_push(skb, IEEE80211_TKIP_IV_LEN);
                ...
                unsigned int mic_len)
{
    ...
    pos = (u8 *)skb_push(skb, IEEE80211_CCMP_HDR_LEN);
...
static int gcmp_encrypt_skb(struct ieee80211_tx_data *tx, struct sk_buff *skb)
{
    ...
    pos = (u8 *)skb_push(skb, IEEE80211_GCMP_HDR_LEN);
    ...
    return ieee80211_aes_gcm_encrypt(key->u.gcmp.tfm, j_0, aad, pos, len,
                     (u8 *)skb_put(skb, IEEE80211_GCMP_MIC_LEN));
}
...
ieee80211_crypto_aes_gmac_decrypt(struct ieee80211_rx_data *rx)
{
    ...
    if (!(status->flag & RX_FLAG_DECRYPTED)) {
        ...
        mic = (u8 *)kmalloc(GMAC_MIC_LEN, GFP_ATOMIC);
        if (!mic)
            return RX_DROP_UNUSABLE;
        if (ieee80211_aes_gmac(key->u.aes_gmac.tfm, aad, nonce,
