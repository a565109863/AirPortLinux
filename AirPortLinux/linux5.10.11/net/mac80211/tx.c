...
ieee80211_tx_h_check_assoc(struct ieee80211_tx_data *tx)
{
    ...
    struct ieee80211_hdr *hdr = (typeof(hdr))tx->skb->data;
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(tx->skb);
    bool assoc = false;
...
//    codel_params_init(&local->cparams);
    ...
    local->cvars = (struct codel_vars *)kcalloc(fq->flows_cnt, sizeof(local->cvars[0]),
                   GFP_KERNEL);
    if (!local->cvars) {
        spin_lock_bh(&fq->lock);
                 ...
                 struct net_device *dev)
{
    ...
    if (unlikely(skb->len < ieee80211_get_radiotap_len((const char *)skb->data)))
                     ...
                     struct net_device *dev)
{
    ...
    len_rthdr = ieee80211_get_radiotap_len((const char *)skb->data);
    ...
    hdr = (typeof(hdr))(skb->data + len_rthdr);
    hdrlen = ieee80211_hdrlen(hdr->frame_control);
                ...
                struct sta_info **sta_out)
{
    ...
    case NL80211_IFTYPE_ADHOC:
        if (is_multicast_ether_addr(skb->data)) {
            *sta_out = (struct sta_info *)ERR_PTR(-ENOENT);
            return 0;
        }
    ...
    *sta_out = sta ?: (struct sta_info *)ERR_PTR(-ENOENT);
    return 0;
}
                       ...
                       u64 *cookie)
{
    ...
    if (head_need > 0 || skb_cloned(skb)) {
        ...
        if (ieee80211_skb_resize(sdata, skb, head_need, ENCRYPT_DATA)) {
            ieee80211_free_txskb(&local->hw, skb);
            skb = NULL;
            return (struct sk_buff *)ERR_PTR(-ENOMEM);
        }
    }
 ...
 free:
    kfree_skb(skb);
    return (struct sk_buff *)ERR_PTR(ret);
}
...
void ieee80211_check_fast_xmit(struct sta_info *sta)
{
    ...
    struct ieee80211_hdr *hdr = (typeof(hdr))build.hdr;
    struct ieee80211_chanctx_conf *chanctx_conf;
    __le16 fc;
    ...
    fast_tx = (struct ieee80211_fast_tx *)kmemdup(&build, sizeof(build), GFP_ATOMIC);
                     ...
                     struct sk_buff *skb)
{
    ...
    memmove(data, (u8 *)data + sizeof(*amsdu_hdr), hdr_len);
    hdr = (typeof(hdr))data;
    amsdu_hdr = (struct ethhdr *)((u8 *)data + hdr_len);
    /* h_80211_src/dst is addr* field within hdr */
    h_80211_src = (u8 *)data + fast_tx->sa_offs;
    h_80211_dst = (u8 *)data + fast_tx->da_offs;
                      ...
                      struct sk_buff *skb)
{
    ...
    memmove(data, (u8 *)data + ETH_ALEN + 2, 2 * ETH_ALEN);

    data = (u8 *)data + 2 * ETH_ALEN;
    len = cpu_to_be16(subframe_len);
    memcpy(data, &len, 2);
    memcpy((u8 *)data + 2, rfc1042_header, sizeof(rfc1042_header));
                ...
                struct sk_buff *skb)
{
    ...
    struct ethhdr eth;
    struct ieee80211_tx_info *info;
    struct ieee80211_hdr *hdr = (typeof(hdr))fast_tx->hdr;
    struct ieee80211_tx_data tx;
    ...
    hdr = (typeof(hdr))skb_push(skb, extra_head);
                       ...
                       struct net_device *dev)
{
    ...
    const struct vlan_ethhdr *ethvlan = (const struct vlan_ethhdr *)skb->data;
    __be16 ethertype;
                  ...
                  struct sk_buff *skb, u32 info_flags)
{
    ...
    if (ieee80211_lookup_ra_sta(sdata, skb, &sta)) {
        kfree_skb(skb);
        skb = (struct sk_buff *)ERR_PTR(-EINVAL);
        goto out;
    }
                     ...
                     struct ieee80211_vif *vif)
{
    ...
    pspoll = (struct ieee80211_pspoll *)skb_put_zero(skb, sizeof(*pspoll));
                       ...
                       bool qos_ok)
{
    ...
    nullfunc = (struct ieee80211_hdr_3addr *)skb_put_zero(skb, sizeof(*nullfunc));
                       ...
                       size_t tailroom)
{
    ...
    hdr = (struct ieee80211_hdr_3addr *)skb_put_zero(skb, sizeof(*hdr));
    hdr->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT |
    ...
    pos = (u8 *)skb_put(skb, ie_ssid_len);
    *pos++ = WLAN_EID_SSID;
    *pos++ = ssid_len;
    if (ssid_len)
