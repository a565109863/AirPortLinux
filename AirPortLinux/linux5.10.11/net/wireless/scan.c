...
static void bss_free(struct cfg80211_internal_bss *bss)
{
    ...
    ies = (struct cfg80211_bss_ies *)rcu_access_pointer(bss->pub.beacon_ies);
    if (ies && !bss->pub.hidden_beacon_bss)
        kfree_rcu(ies, rcu_head);
    ies = (struct cfg80211_bss_ies *)rcu_access_pointer(bss->pub.proberesp_ies);
    if (ies)
        kfree_rcu(ies, rcu_head);
                  ...
                  u8 *new_ie, gfp_t gfp)
{
     ...
     */
    sub_copy = (u8 *)kmemdup(subelement, subie_len, gfp);
    if (!sub_copy)
        return 0;
    ...
    while (tmp_old + tmp_old[1] + 2 - ie <= ielen) {
        ...
        if (!tmp) {
            const struct element *old_elem = (const struct element *)tmp_old;
                       ...
                       struct list_head *list)
{
    ...
    while (pos + sizeof(*ap_info) <= end) {
        ...
        for (i = 0; i < count; i++) {
            ...
            entry = (struct cfg80211_colocated_ap *)kzalloc(sizeof(*entry) + IEEE80211_MAX_SSID_LEN,
                    GFP_ATOMIC);
...
static int cfg80211_scan_6ghz(struct cfg80211_registered_device *rdev)
{
    ...
    request = (struct cfg80211_scan_request *)kzalloc(struct_size(request, channels, n_channels) +
    ...
    *request = *rdev_req;
    ...
    request->scan_6ghz_params =
        (typeof(request->scan_6ghz_params))&request->channels[n_channels];
...
skip:
    ...
    if (request->n_channels) {
        struct cfg80211_scan_request *old = (typeof(old))rdev->int_scan_req;

        rdev->int_scan_req = (typeof(rdev->int_scan_req))request;
...
int cfg80211_scan(struct cfg80211_registered_device *rdev)
{
    ...
    request = (typeof(request))kzalloc(struct_size(request, channels, n_channels),
              GFP_KERNEL);
    if (!request)
        return -ENOMEM;
...
static bool cfg80211_combine_bsses(struct cfg80211_registered_device *rdev,
                   struct cfg80211_internal_bss *_new)
{
    const struct cfg80211_bss_ies *ies;
    struct cfg80211_internal_bss *bss;
    const u8 *ie;
    ...
    ies = rcu_access_pointer(_new->pub.beacon_ies);
    if (WARN_ON(!ies))
        return false;
    ...
    list_for_each_entry(bss, &rdev->bss_list, list) {
        ...
        if (!ether_addr_equal(bss->pub.bssid, _new->pub.bssid))
            continue;
        if (bss->pub.channel != _new->pub.channel)
            continue;
        if (bss->pub.scan_width != _new->pub.scan_width)
            continue;
        ...
        if (WARN_ON_ONCE(!list_empty(&bss->hidden_list)))
            list_del(&bss->hidden_list);
        /* combine them */
        list_add(&bss->hidden_list, &_new->hidden_list);
        bss->pub.hidden_beacon_bss = &_new->pub;
        _new->refcount += bss->refcount;
              ...
              bool signal_valid)
{
    ...
    if (rcu_access_pointer(_new->pub.proberesp_ies)) {
        ...
        if (known->pub.hidden_beacon_bss &&
            ...
            f = rcu_access_pointer(_new->pub.beacon_ies);
        ...
        rcu_assign_pointer(known->pub.beacon_ies, _new->pub.beacon_ies);

        /* Override IEs if they were from a beacon before */
        if (old == rcu_access_pointer(known->pub.ies))
            rcu_assign_pointer(known->pub.ies, _new->pub.beacon_ies);
    ...
    known->pub.beacon_interval = _new->pub.beacon_interval;

    /* don't update the signal if beacon was heard on
     * adjacent channel.
     */
    if (signal_valid)
        known->pub.signal = _new->pub.signal;
    known->pub.capability = _new->pub.capability;
    known->ts = _new->ts;
    known->ts_boottime = _new->ts_boottime;
    known->parent_tsf = _new->parent_tsf;
    known->pub.chains = _new->pub.chains;
    memcpy(known->pub.chain_signal, _new->pub.chain_signal,
           IEEE80211_MAX_CHAINS);
    ether_addr_copy(known->parent_bssid, _new->parent_bssid);
    known->pub.max_bssid_indicator = _new->pub.max_bssid_indicator;
    known->pub.bssid_index = _new->pub.bssid_index;
            ...
            bool signal_valid, unsigned long ts)
{
    ...
    if (found) {
         ...
         */
        _new = (typeof(_new))kzalloc(sizeof(*_new) + rdev->wiphy.bss_priv_size,
                  GFP_ATOMIC);
        if (!_new) {
            ies = (typeof(ies))rcu_dereference(tmp->pub.beacon_ies);
            if (ies)
                kfree_rcu(ies, rcu_head);
            ies = (typeof(ies))rcu_dereference(tmp->pub.proberesp_ies);
            if (ies)
                kfree_rcu(ies, rcu_head);
            goto drop;
        }
        memcpy(_new, tmp, sizeof(*_new));
        _new->refcount = 1;
        INIT_LIST_HEAD(&_new->hidden_list);
        INIT_LIST_HEAD(&_new->pub.nontrans_list);
        ...
        if (rcu_access_pointer(tmp->pub.proberesp_ies)) {
            ...
            if (!hidden)
                hidden = rb_find_bss(rdev, tmp,
                             BSS_CMP_HIDE_NUL);
            if (hidden) {
                _new->pub.hidden_beacon_bss = &hidden->pub;
                list_add(&_new->hidden_list,
                     &hidden->hidden_list);
                hidden->refcount++;
                rcu_assign_pointer(_new->pub.beacon_ies,
             ...
             */
            if (!cfg80211_combine_bsses(rdev, _new)) {
                kfree(_new);
                goto drop;
            }
        }
        ...
        if (tmp->pub.transmitted_bss) {
            ...
            _new->pub.transmitted_bss = tmp->pub.transmitted_bss;
            bss_ref_get(rdev, pbss);
        }

        list_add_tail(&_new->list, &rdev->bss_list);
        rdev->bss_entries++;
        rb_insert_bss(rdev, _new);
        found = _new;
    }
             ...
             enum nl80211_bss_scan_width scan_width)
{
    ...
    if (channel->band == NL80211_BAND_S1GHZ) {
        ...
        if (tmp && tmp[1] >= sizeof(struct ieee80211_s1g_oper_ie)) {
            struct ieee80211_s1g_oper_ie *s1gop = (typeof(s1gop))(tmp + 2);
        ...
        if (tmp && tmp[1] == 1) {
            ...
            if (tmp && tmp[1] >= sizeof(struct ieee80211_ht_operation)) {
                struct ieee80211_ht_operation *htop = (typeof(htop))(tmp + 2);
                ...
                gfp_t gfp)
{
     ...
     */
    ies = (typeof(ies))kzalloc(sizeof(*ies) + ielen, gfp);
    if (!ies)
        return NULL;
    ies->len = ielen;
    ies->tsf = tsf;
                   ...
                   const struct element *sub_elem)
{
    ...
    */
    ...
    next_sub = (typeof(next_sub))&next_mbssid->data[1];
                  ...
                  u8 *merged_ie, size_t max_copy_len)
{
    ...
    while ((next_mbssid = cfg80211_get_profile_continuation(ie, ielen,
                                mbssid_elem,
                                sub_elem))) {
        const struct element *next_sub = (typeof(next_sub))&next_mbssid->data[1];
                       ...
                       gfp_t gfp)
{
    ...
    new_ie = (typeof(new_ie))kmalloc(IEEE80211_MAX_DATA_LEN, gfp);
    if (!new_ie)
        return;

    profile = (typeof(profile))kmalloc(ielen, gfp);
    if (!profile)
        goto out;
                   ...
                   struct ieee80211_mgmt *mgmt, size_t len)
{
     ...
     * 2. skip MBSSID IE
     */
    new_ie = (typeof(new_ie))kzalloc(new_ie_len, GFP_ATOMIC);
    if (!new_ie)
        return;

    new_ies = (typeof(new_ies))kzalloc(sizeof(*new_ies) + new_ie_len, GFP_ATOMIC);
    if (!new_ies)
        goto out_free;
                      ...
                      gfp_t gfp)
{
    ...
    if (ext) {
        ...
        ie = (typeof(ie))cfg80211_find_ie(WLAN_EID_S1G_BCN_COMPAT,
                          variable, ielen);
        if (!ie)
            return NULL;
        compat = (typeof(compat))(ie + 2);
                     ...
                     struct ieee80211_channel *chan)
{
    ...
    struct cfg80211_internal_bss *_new = NULL;
    struct cfg80211_internal_bss *bss;
    struct cfg80211_bss *nontrans_bss;
    ...
    if (_new) {
        ...
        if (cfg80211_update_known_bss(rdev, cbss, _new, false)) {
            _new->pub.proberesp_ies = NULL;
            _new->pub.beacon_ies = NULL;
        }

        list_for_each_entry_safe(nontrans_bss, tmp,
                     &_new->pub.nontrans_list,
                     nontrans_list) {
        ...
        WARN_ON(atomic_read(&_new->hold));
        if (!WARN_ON(!__cfg80211_unlink_bss(rdev, _new)))
            rdev->bss_generation++;
    }
              ...
              union iwreq_data *wrqu, char *extra)
{
    ...
    creq = (typeof(creq))kzalloc(sizeof(*creq) + sizeof(struct cfg80211_ssid) +
    ...
    creq->ssids = (typeof(creq->ssids))&creq->channels[n_channels];
