...
enum nl80211_multicast_groups {
 ...
 const struct genl_multicast_group nl80211_mcgrps[] = {
...
__cfg80211_wdev_from_attrs(struct net *netns, struct nlattr **attrs)
{
    ...
    if (!have_ifidx && !have_wdev_id)
        return (struct wireless_dev *)ERR_PTR(-EINVAL);
    ...
    if (result)
        return result;
    return (struct wireless_dev *)ERR_PTR(-ENODEV);
}
...
__cfg80211_rdev_from_attrs(struct net *netns, struct nlattr **attrs)
{
    ...
    if (!attrs[NL80211_ATTR_WIPHY] &&
        ...
        return (struct cfg80211_registered_device *)ERR_PTR(-EINVAL);
...
{
    const u8 *data = (typeof(data))nla_data(attr);
    unsigned int len = nla_len(attr);
    const struct element *elem;
    const struct ieee80211_mgmt *mgmt = (typeof(mgmt))data;
                  ...
                  struct wireless_dev **wdev)
{
    ...
    if (!cb->args[0]) {
        ...
        attrbuf = (typeof(attrbuf))kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf),
                  GFP_KERNEL);
        if (!attrbuf)
            return -ENOMEM;
                 ...
                 struct key_parse *k)
{
    ...
    if (tb[NL80211_KEY_DATA]) {
        k->p.key = (typeof(k->p.key))nla_data(tb[NL80211_KEY_DATA]);
        k->p.key_len = nla_len(tb[NL80211_KEY_DATA]);
    }

    if (tb[NL80211_KEY_SEQ]) {
        k->p.seq = (typeof(k->p.seq))nla_data(tb[NL80211_KEY_SEQ]);
    ...
    if (tb[NL80211_KEY_MODE])
        k->p.mode = (typeof(k->p.mode))nla_get_u8(tb[NL80211_KEY_MODE]);
...
static int nl80211_parse_key_old(struct genl_info *info, struct key_parse *k)
{
    if (info->attrs[NL80211_ATTR_KEY_DATA]) {
        k->p.key = (typeof(k->p.key))nla_data(info->attrs[NL80211_ATTR_KEY_DATA]);
        k->p.key_len = nla_len(info->attrs[NL80211_ATTR_KEY_DATA]);
    }

    if (info->attrs[NL80211_ATTR_KEY_SEQ]) {
        k->p.seq = (typeof(k->p.seq))nla_data(info->attrs[NL80211_ATTR_KEY_SEQ]);
               ...
               struct genl_info *info, bool *no_ht)
{
    ...
    result = (typeof(result))kzalloc(sizeof(*result), GFP_KERNEL);
    if (!result)
        return (struct cfg80211_cached_keys *)ERR_PTR(-ENOMEM);
 ...
 error:
    kfree(result);
    return (struct cfg80211_cached_keys *)ERR_PTR(err);
}
...
{
    struct nlattr **tb = (typeof(tb))kcalloc(NUM_NL80211_ATTR, sizeof(*tb), GFP_KERNEL);
    int ret;
...
{
    int idx = 0, ret;
    struct nl80211_dump_wiphy_state *state = (typeof(state))cb->args[0];
    struct cfg80211_registered_device *rdev;

    rtnl_lock();
    if (!state) {
        state = (typeof(state))kzalloc(sizeof(*state), GFP_KERNEL);
        if (!state) {
            rtnl_unlock();
            return -ENOMEM;
        }
...
{
    struct sk_buff *msg;
    struct cfg80211_registered_device *rdev = (typeof(rdev))info->user_ptr[0];
    struct nl80211_dump_wiphy_state state = {};
                ...
                struct ieee80211_txq_params *txq_params)
{
    ...
    if (ac >= NL80211_NUM_ACS)
        return -EINVAL;
    txq_params->ac = (enum nl80211_ac)array_index_nospec(ac, NL80211_NUM_ACS);
    return 0;
}
              ...
              struct cfg80211_chan_def *chandef)
{
    ...
    if (attrs[NL80211_ATTR_WIPHY_CHANNEL_TYPE]) {
        ...
        chantype = (enum nl80211_channel_type)nla_get_u32(attrs[NL80211_ATTR_WIPHY_CHANNEL_TYPE]);
        ...
        chandef->width =
        (typeof(chandef->width))nla_get_u32(attrs[NL80211_ATTR_CHANNEL_WIDTH]);
        if (attrs[NL80211_ATTR_CENTER_FREQ1]) {
    ...
    if (info->attrs[NL80211_ATTR_WIPHY_EDMG_CHANNELS]) {
        ...
        if (info->attrs[NL80211_ATTR_WIPHY_EDMG_BW_CONFIG])
            chandef->edmg.bw_config =
             (typeof(chandef->edmg.bw_config))nla_get_u8(info->attrs[NL80211_ATTR_WIPHY_EDMG_BW_CONFIG]);
    } else {
        chandef->edmg.bw_config = (typeof(chandef->edmg.bw_config))0;
...
{
    struct cfg80211_registered_device *rdev = (typeof(rdev))info->user_ptr[0];
    struct net_device *netdev = (typeof(netdev))info->user_ptr[1];
...
{
    struct cfg80211_registered_device *rdev = (typeof(rdev))info->user_ptr[0];
    struct net_device *dev = (typeof(dev))info->user_ptr[1];
    ...
    bssid = (typeof(bssid))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_set_wiphy(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_WIPHY_NAME])
        result = cfg80211_dev_rename(
            rdev, (char *)nla_data(info->attrs[NL80211_ATTR_WIPHY_NAME]));
    ...
    if (info->attrs[NL80211_ATTR_WIPHY_TX_POWER_SETTING]) {
        ...
        type = (typeof(type))nla_get_u32(info->attrs[idx]);
...
static int nl80211_get_interface(struct sk_buff *skb, struct genl_info *info)
{
    struct sk_buff *msg;
    struct cfg80211_registered_device *rdev = (typeof(rdev))info->user_ptr[0];
    struct wireless_dev *wdev = (typeof(wdev))info->user_ptr[1];
                     ...
                     struct vif_params *params)
{
    ...
    if (info->attrs[NL80211_ATTR_MU_MIMO_GROUP_DATA]) {
        ...
        if (!wiphy_ext_feature_isset(&rdev->wiphy, (enum nl80211_ext_feature_index)cap_flag))
            return -EOPNOTSUPP;

        mumimo_groups =
            (typeof(mumimo_groups))nla_data(info->attrs[NL80211_ATTR_MU_MIMO_GROUP_DATA]);
    ...
    if (info->attrs[NL80211_ATTR_MU_MIMO_FOLLOW_MAC_ADDR]) {
        ...
        params->vht_mumimo_follow_addr =
            (typeof(params->vht_mumimo_follow_addr))nla_data(info->attrs[NL80211_ATTR_MU_MIMO_FOLLOW_MAC_ADDR]);
        change = true;
    }
...
static int nl80211_set_interface(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IFTYPE]) {
        ntype = (typeof(ntype))nla_get_u32(info->attrs[NL80211_ATTR_IFTYPE]);
        if (otype != ntype)
            change = true;
    }
...
static int nl80211_new_interface(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IFTYPE])
        type = (typeof(type))nla_get_u32(info->attrs[NL80211_ATTR_IFTYPE]);
    ...
    wdev = rdev_add_virtual_intf(rdev,
                (char *)nla_data(info->attrs[NL80211_ATTR_IFNAME]),
...
{
    struct nlattr *key;
    struct get_key_cookie *cookie = (typeof(cookie))c;
...
static int nl80211_get_key(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        mac_addr = (typeof(mac_addr))nla_data(info->attrs[NL80211_ATTR_MAC]);
                        ...
                        struct genl_info *info)
{
    ...
    if (!wiphy->max_acl_mac_addrs)
        return (struct cfg80211_acl_data *)ERR_PTR(-EOPNOTSUPP);

    if (!info->attrs[NL80211_ATTR_ACL_POLICY])
        return (struct cfg80211_acl_data *)ERR_PTR(-EINVAL);

    acl_policy = (typeof(acl_policy))nla_get_u32(info->attrs[NL80211_ATTR_ACL_POLICY]);
    ...
    if (n_entries < 0)
        return (struct cfg80211_acl_data *)ERR_PTR(n_entries);

    if (n_entries > wiphy->max_acl_mac_addrs)
        return (struct cfg80211_acl_data *)ERR_PTR(-ENOTSUPP);

    acl = (typeof(acl))kzalloc(struct_size(acl, mac_addrs, n_entries), GFP_KERNEL);
    if (!acl)
        return (struct cfg80211_acl_data *)ERR_PTR(-ENOMEM);
                     ...
                     struct net_device *dev)
{
    ...
    for (i = 0; i < NUM_NL80211_BANDS; i++) {
        ...
        mask->control[i].he_gi = (typeof(mask->control[i].he_gi))0xFF;
        mask->control[i].he_ltf = (typeof(mask->control[i].he_ltf))0xFF;
    }
    ...
    nla_for_each_nested(tx_rates, attrs[attr], rem) {
        enum nl80211_band band = (typeof(band))nla_type(tx_rates);
        int err;
        ...
        if (tb[NL80211_TXRATE_LEGACY]) {
            mask->control[band].legacy = rateset_to_mask(
                sband,
                (u8 *)nla_data(tb[NL80211_TXRATE_LEGACY]),
        ...
        if (tb[NL80211_TXRATE_HT]) {
            if (!ht_rateset_to_mask(
                    sband,
                    (u8 *)nla_data(tb[NL80211_TXRATE_HT]),
        ...
        if (tb[NL80211_TXRATE_VHT]) {
            if (!vht_set_mcs_mask(
                    sband,
                    (struct nl80211_txrate_vht *)nla_data(tb[NL80211_TXRATE_VHT]),
                    mask->control[band].vht_mcs))
                return -EINVAL;
        }
        if (tb[NL80211_TXRATE_GI]) {
            mask->control[band].gi =
            (typeof(mask->control[band].gi))nla_get_u8(tb[NL80211_TXRATE_GI]);
            if (mask->control[band].gi > NL80211_TXRATE_FORCE_LGI)
                return -EINVAL;
        }
        if (tb[NL80211_TXRATE_HE] &&
            !he_set_mcs_mask(info, wdev, sband,
                     (struct nl80211_txrate_he *)nla_data(tb[NL80211_TXRATE_HE]),
                     mask->control[band].he_mcs))
            return -EINVAL;
        if (tb[NL80211_TXRATE_HE_GI])
            mask->control[band].he_gi =
                (typeof(mask->control[band].he_gi))nla_get_u8(tb[NL80211_TXRATE_HE_GI]);
        if (tb[NL80211_TXRATE_HE_LTF])
            mask->control[band].he_ltf =
                (typeof(mask->control[band].he_ltf))nla_get_u8(tb[NL80211_TXRATE_HE_LTF]);
                ...
                struct cfg80211_beacon_data *bcn)
{
    ...
    if (attrs[NL80211_ATTR_BEACON_HEAD]) {
        bcn->head = (typeof(bcn->head))nla_data(attrs[NL80211_ATTR_BEACON_HEAD]);
    ...
    if (attrs[NL80211_ATTR_BEACON_TAIL]) {
        bcn->tail = (typeof(bcn->tail))nla_data(attrs[NL80211_ATTR_BEACON_TAIL]);
    ...
    if (attrs[NL80211_ATTR_IE]) {
        bcn->beacon_ies = (typeof(bcn->beacon_ies))nla_data(attrs[NL80211_ATTR_IE]);
        bcn->beacon_ies_len = nla_len(attrs[NL80211_ATTR_IE]);
    }

    if (attrs[NL80211_ATTR_IE_PROBE_RESP]) {
        bcn->proberesp_ies =
        (typeof(bcn->proberesp_ies))nla_data(attrs[NL80211_ATTR_IE_PROBE_RESP]);
    ...
    if (attrs[NL80211_ATTR_IE_ASSOC_RESP]) {
        bcn->assocresp_ies =
        (typeof(bcn->assocresp_ies))nla_data(attrs[NL80211_ATTR_IE_ASSOC_RESP]);
        bcn->assocresp_ies_len =
            nla_len(attrs[NL80211_ATTR_IE_ASSOC_RESP]);
    }

    if (attrs[NL80211_ATTR_PROBE_RESP]) {
        bcn->probe_resp = (typeof(bcn->probe_resp))nla_data(attrs[NL80211_ATTR_PROBE_RESP]);
    ...
    if (attrs[NL80211_ATTR_FTM_RESPONDER]) {
        ...
        if (tb[NL80211_FTM_RESP_ATTR_LCI]) {
            bcn->lci = (typeof(bcn->lci))nla_data(tb[NL80211_FTM_RESP_ATTR_LCI]);
            bcn->lci_len = nla_len(tb[NL80211_FTM_RESP_ATTR_LCI]);
        }

        if (tb[NL80211_FTM_RESP_ATTR_CIVICLOC]) {
            bcn->civicloc = (typeof(bcn->civicloc))nla_data(tb[NL80211_FTM_RESP_ATTR_CIVICLOC]);
                    ...
                    struct cfg80211_ap_settings *params)
{
    ...
    fd->tmpl = (typeof(fd->tmpl))nla_data(tb[NL80211_FILS_DISCOVERY_ATTR_TMPL]);
                     ...
                     struct cfg80211_ap_settings *params)
{
    ...
    presp->tmpl = (typeof(presp->tmpl))nla_data(tb[NL80211_UNSOL_BCAST_PROBE_RESP_ATTR_TMPL]);
...
static void nl80211_calculate_ap_params(struct cfg80211_ap_settings *params)
{
    ...
    if (cap && cap[1] >= sizeof(*params->ht_cap))
        params->ht_cap = (typeof(params->ht_cap))(cap + 2);
    cap = cfg80211_find_ie(WLAN_EID_VHT_CAPABILITY, ies, ies_len);
    if (cap && cap[1] >= sizeof(*params->vht_cap))
        params->vht_cap = (typeof(params->vht_cap))(cap + 2);
    cap = cfg80211_find_ext_ie(WLAN_EID_EXT_HE_CAPABILITY, ies, ies_len);
    if (cap && cap[1] >= sizeof(*params->he_cap) + 1)
        params->he_cap = (typeof(params->he_cap))(cap + 3);
    cap = cfg80211_find_ext_ie(WLAN_EID_EXT_HE_OPERATION, ies, ies_len);
    if (cap && cap[1] >= sizeof(*params->he_oper) + 1)
        params->he_oper = (typeof(params->he_oper))(cap + 3);
}
...
static int nl80211_start_ap(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_SSID]) {
        params.ssid = (typeof(params.ssid))nla_data(info->attrs[NL80211_ATTR_SSID]);
        params.ssid_len =
    ...
    if (info->attrs[NL80211_ATTR_HIDDEN_SSID])
        params.hidden_ssid = (typeof(params.hidden_ssid))nla_get_u32(
            info->attrs[NL80211_ATTR_HIDDEN_SSID]);

    params.privacy = !!info->attrs[NL80211_ATTR_PRIVACY];

    if (info->attrs[NL80211_ATTR_AUTH_TYPE]) {
        params.auth_type = (typeof(params.auth_type))nla_get_u32(
    ...
    if (info->attrs[NL80211_ATTR_SMPS_MODE]) {
        params.smps_mode =
        (typeof(params.smps_mode))nla_get_u8(info->attrs[NL80211_ATTR_SMPS_MODE]);
                   ...
                   struct station_parameters *params)
{
    ...
    if (nla) {
        ...
        sta_flags = (typeof(sta_flags))nla_data(nla);
                   ...
                   struct cfg80211_registered_device *rdev)
{
    ...
    if (!v)
        return (struct net_device *)ERR_PTR(-ENODEV);
 ...
 error:
    dev_put(v);
    return (struct net_device *)ERR_PTR(ret);
}
                      ...
                      struct station_parameters *params)
{
    if (info->attrs[NL80211_ATTR_STA_SUPPORTED_CHANNELS]) {
        ...
        (typeof(params->supported_channels))nla_data(info->attrs[NL80211_ATTR_STA_SUPPORTED_CHANNELS]);
    ...
    if (info->attrs[NL80211_ATTR_STA_SUPPORTED_OPER_CLASSES]) {
        ...
        (typeof(params->supported_oper_classes))nla_data(info->attrs[NL80211_ATTR_STA_SUPPORTED_OPER_CLASSES]);
                    ...
                    struct station_parameters *params)
{
    ...
    if (info->attrs[NL80211_ATTR_HT_CAPABILITY])
        params->ht_capa =
        (typeof(params->ht_capa))nla_data(info->attrs[NL80211_ATTR_HT_CAPABILITY]);
    if (info->attrs[NL80211_ATTR_VHT_CAPABILITY])
        params->vht_capa =
        (typeof(params->vht_capa))nla_data(info->attrs[NL80211_ATTR_VHT_CAPABILITY]);
    if (info->attrs[NL80211_ATTR_HE_CAPABILITY]) {
        params->he_capa =
        (typeof(params->he_capa))nla_data(info->attrs[NL80211_ATTR_HE_CAPABILITY]);
        params->he_capa_len =
                         ...
                         struct station_parameters *params)
{
    ...
    if (info->attrs[NL80211_ATTR_STA_TX_POWER_SETTING]) {
        ...
        params->txpwr.type = (typeof(params->txpwr.type))nla_get_u8(info->attrs[idx]);
...
static int nl80211_set_station(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_STA_SUPPORTED_RATES]) {
        params.supported_rates =
        (typeof(params.supported_rates))nla_data(info->attrs[NL80211_ATTR_STA_SUPPORTED_RATES]);
    ...
    if (info->attrs[NL80211_ATTR_STA_EXT_CAPABILITY]) {
        params.ext_capab =
        (typeof(params.ext_capab))nla_data(info->attrs[NL80211_ATTR_STA_EXT_CAPABILITY]);
    ...
    if (info->attrs[NL80211_ATTR_LOCAL_MESH_POWER_MODE])
        params.local_pm = (typeof(params.local_pm))nla_get_u32(
    ...
    if (info->attrs[NL80211_ATTR_HE_6GHZ_CAPABILITY])
        params.he_6ghz_capa =
        (typeof(params.he_6ghz_capa))nla_data(info->attrs[NL80211_ATTR_HE_6GHZ_CAPABILITY]);
...
static int nl80211_del_station(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        params.mac = (typeof(params.mac))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_get_mpath(struct sk_buff *skb, struct genl_info *info)
{
    ...
    dst = (typeof(dst))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_set_mpath(struct sk_buff *skb, struct genl_info *info)
{
    ...
    dst = (typeof(dst))nla_data(info->attrs[NL80211_ATTR_MAC]);
    next_hop = (typeof(next_hop))nla_data(info->attrs[NL80211_ATTR_MPATH_NEXT_HOP]);
...
static int nl80211_set_bss(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_BSS_BASIC_RATES]) {
        params.basic_rates =
            (typeof(params.basic_rates))nla_data(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
...
static int nl80211_req_set_reg(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_USER_REG_HINT_TYPE])
        user_reg_hint_type =
        (typeof(user_reg_hint_type))nla_get_u32(info->attrs[NL80211_ATTR_USER_REG_HINT_TYPE]);
    else
    ...
    case NL80211_USER_REG_HINT_CELL_BASE:
        ...
        data = (typeof(data))nla_data(info->attrs[NL80211_ATTR_REG_ALPHA2]);
...
do {                                    \
    ...
    FILL_IN_MESH_PARAM_IF_SET(tb, cfg, power_mode, mask,
                  NL80211_MESHCONF_POWER_MODE, (enum nl80211_mesh_power_mode)nla_get_u32);
                     ...
                     struct mesh_setup *setup)
{
    ...
    if (tb[NL80211_MESH_SETUP_IE]) {
        ...
        setup->ie = (typeof(setup->ie))nla_data(ieattr);
...
static int nl80211_set_reg(struct sk_buff *skb, struct genl_info *info)
{
    ...
    alpha2 = (typeof(mac_addr))nla_data(info->attrs[NL80211_ATTR_REG_ALPHA2]);
                ...
                struct cfg80211_bss_selection *bss_select)
{
    ...
    if (attr[NL80211_BSS_SELECT_ATTR_BAND_PREF]) {
        ...
        bss_select->param.band_pref =
            (typeof(bss_select->param.band_pref))nla_get_u32(attr[NL80211_BSS_SELECT_ATTR_BAND_PREF]);
    ...
    if (attr[NL80211_BSS_SELECT_ATTR_RSSI_ADJUST]) {
        ...
        adj_param = (typeof(adj_param))nla_data(attr[NL80211_BSS_SELECT_ATTR_RSSI_ADJUST]);
        bss_select->behaviour = NL80211_BSS_SELECT_ATTR_RSSI_ADJUST;
        bss_select->param.adjust.band = (typeof(bss_select->param.adjust.band))adj_param->band;
             ...
             bool is_sched_scan)
{
    ...
    if (is_sched_scan) {
        struct cfg80211_sched_scan_request *req = (typeof(req))request;
        ...
        struct cfg80211_scan_request *req = (typeof(req))request;
...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
    ...
    request = (typeof(request))kzalloc(sizeof(*request)
    ...
    if (n_ssids)
        request->ssids = (typeof(request->ssids))&request->channels[n_channels];
    request->n_ssids = n_ssids;
    if (ie_len) {
        if (n_ssids)
            request->ie = (typeof(request->ie))(request->ssids + n_ssids);
        else
            request->ie = (typeof(request->ie))(request->channels + n_channels);
    }
    ...
    if (info->attrs[NL80211_ATTR_SCAN_SUPP_RATES]) {
        nla_for_each_nested(attr,
                    ...
                    tmp) {
            enum nl80211_band band = (typeof(band))nla_type(attr);
             ...
             struct nlattr **attrs, int max_match_sets)
{
    ...
    if (attrs[NL80211_ATTR_SCAN_FREQUENCIES]) {
        ...
        if (!n_channels)
            return (struct cfg80211_sched_scan_request *)ERR_PTR(-EINVAL);
    } else {
        n_channels = ieee80211_get_num_supported_channels(wiphy);
    }
    ...
    if (attrs[NL80211_ATTR_SCHED_SCAN_MATCH]) {
        nla_for_each_nested(attr,
            ...
            err = nla_parse_nested_deprecated(tb,
                              ...
                              NULL);
            if (err)
                return (struct cfg80211_sched_scan_request *)ERR_PTR(err);
    ...
    request = (typeof(request))kzalloc(sizeof(*request)
            ...
            + ie_len, GFP_KERNEL);
    if (!request)
        return (struct cfg80211_sched_scan_request *)ERR_PTR(-ENOMEM);
    ...
    if (n_match_sets) {
        if (request->ie)
            request->match_sets = (typeof(request->match_sets))(request->ie + ie_len);
        else if (n_ssids)
            request->match_sets =
                (typeof(request->match_sets))(request->ssids + n_ssids);
        else
            request->match_sets =
                (typeof(request->match_sets))(request->channels + n_channels);
    }
    request->n_match_sets = n_match_sets;

    if (n_match_sets)
        request->scan_plans = (typeof(request->scan_plans))(request->match_sets +
                           n_match_sets);
    else if (request->ie)
        request->scan_plans = (typeof(request->scan_plans))(request->ie + ie_len);
    else if (n_ssids)
        request->scan_plans = (typeof(request->scan_plans))(request->ssids + n_ssids);
    else
        request->scan_plans = (typeof(request->scan_plans))(request->channels + n_channels);
    ...
    if (request->relative_rssi_set &&
        ...
        rssi_adjust = (typeof(rssi_adjust))nla_data(
            attrs[NL80211_ATTR_SCHED_SCAN_RSSI_ADJUST]);
        request->rssi_adjust.band = (typeof(request->rssi_adjust.band))rssi_adjust->band;
...
static int nl80211_channel_switch(struct sk_buff *skb, struct genl_info *info)
{
    ...
    params.counter_offsets_beacon =
        (typeof(params.counter_offsets_beacon))nla_data(csa_attrs[NL80211_ATTR_CNTDWN_OFFS_BEACON]);
    ...
    if (csa_attrs[NL80211_ATTR_CNTDWN_OFFS_PRESP]) {
        ...
        params.counter_offsets_presp =
            (typeof(params.counter_offsets_presp))nla_data(csa_attrs[NL80211_ATTR_CNTDWN_OFFS_PRESP]);
...
static int nl80211_dump_survey(struct sk_buff *skb, struct netlink_callback *cb)
{
    ...
    attrbuf = (typeof(attrbuf))kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf), GFP_KERNEL);
    if (!attrbuf)
        return -ENOMEM;
...
static int nl80211_authenticate(struct sk_buff *skb, struct genl_info *info)
{
    ...
    ssid = (typeof(ssid))nla_data(info->attrs[NL80211_ATTR_SSID]);
    ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);

    if (info->attrs[NL80211_ATTR_IE]) {
        ie = (typeof(ie))nla_data(info->attrs[NL80211_ATTR_IE]);
        ie_len = nla_len(info->attrs[NL80211_ATTR_IE]);
    }

    auth_type = (typeof(auth_type))nla_get_u32(info->attrs[NL80211_ATTR_AUTH_TYPE]);
    ...
    if (info->attrs[NL80211_ATTR_AUTH_DATA]) {
        if (auth_type != NL80211_AUTHTYPE_SAE &&
            ...
            auth_type != NL80211_AUTHTYPE_FILS_PK)
            return -EINVAL;
        auth_data = (typeof(auth_data))nla_data(info->attrs[NL80211_ATTR_AUTH_DATA]);
                   ...
                   int cipher_limit)
{
    ...
    if (info->attrs[NL80211_ATTR_PMK]) {
        ...
        if (!wiphy_ext_feature_isset(&rdev->wiphy,
            ...
            return -EINVAL;
        settings->psk = (typeof(settings->psk))nla_data(info->attrs[NL80211_ATTR_PMK]);
    }
    ...
    if (info->attrs[NL80211_ATTR_SAE_PASSWORD]) {
        if (!wiphy_ext_feature_isset(&rdev->wiphy,
            ...
            return -EINVAL;
        settings->sae_pwd =
            (typeof(settings->sae_pwd))nla_data(info->attrs[NL80211_ATTR_SAE_PASSWORD]);
...
static int nl80211_associate(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IE]) {
        req.ie = (typeof(req.ie))nla_data(info->attrs[NL80211_ATTR_IE]);
        req.ie_len = nla_len(info->attrs[NL80211_ATTR_IE]);
    }

    if (info->attrs[NL80211_ATTR_USE_MFP]) {
        enum nl80211_mfp mfp =
            (typeof(mfp))nla_get_u32(info->attrs[NL80211_ATTR_USE_MFP]);
    ...
    if (info->attrs[NL80211_ATTR_PREV_BSSID])
        req.prev_bssid = (typeof(req.prev_bssid))nla_data(info->attrs[NL80211_ATTR_PREV_BSSID]);
    ...
    if (info->attrs[NL80211_ATTR_FILS_KEK]) {
        req.fils_kek = (typeof(req.fils_kek))nla_data(info->attrs[NL80211_ATTR_FILS_KEK]);
        req.fils_kek_len = nla_len(info->attrs[NL80211_ATTR_FILS_KEK]);
        if (!info->attrs[NL80211_ATTR_FILS_NONCES])
            return -EINVAL;
        req.fils_nonces =
        (typeof(req.fils_nonces))nla_data(info->attrs[NL80211_ATTR_FILS_NONCES]);
    }
...
static int nl80211_join_ibss(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_MAC]) {
        ibss.bssid = (typeof(ibss.bssid))nla_data(info->attrs[NL80211_ATTR_MAC]);

        if (!is_valid_ether_addr(ibss.bssid))
            return -EINVAL;
    }
    ibss.ssid = (typeof(ibss.ssid))nla_data(info->attrs[NL80211_ATTR_SSID]);
    ibss.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);

    if (info->attrs[NL80211_ATTR_IE]) {
        ibss.ie = (typeof(ibss.ie))nla_data(info->attrs[NL80211_ATTR_IE]);
    ...
    if (info->attrs[NL80211_ATTR_BSS_BASIC_RATES]) {
        u8 *rates =
            (typeof(rates))nla_data(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        int n_rates =
            nla_len(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        struct ieee80211_supported_band *sband =
...
{
    struct cfg80211_registered_device *rdev = (typeof(rdev))((void **)skb->cb)[0];
    void *hdr = ((void **)skb->cb)[1];
    struct nlmsghdr *nlhdr = nlmsg_hdr(skb);
    struct nlattr *data = (typeof(data))((void **)skb->cb)[2];
                 ...
                 struct netlink_callback *cb)
{
    ...
    if (cb->args[1]) {
        data = (typeof(mac_addr))nla_data((void *)cb->args[1]);
...
static int nl80211_connect(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_AUTH_TYPE]) {
        connect.auth_type =
            (typeof(connect.auth_type))nla_get_u32(info->attrs[NL80211_ATTR_AUTH_TYPE]);
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        connect.bssid = (typeof(connect.bssid))nla_data(info->attrs[NL80211_ATTR_MAC]);
    else if (info->attrs[NL80211_ATTR_MAC_HINT])
        connect.bssid_hint =
        (typeof(connect.bssid_hint))nla_data(info->attrs[NL80211_ATTR_MAC_HINT]);
    connect.ssid = (typeof(connect.ssid))nla_data(info->attrs[NL80211_ATTR_SSID]);
    connect.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);

    if (info->attrs[NL80211_ATTR_IE]) {
        connect.ie = (typeof(connect.ie))nla_data(info->attrs[NL80211_ATTR_IE]);
        connect.ie_len = nla_len(info->attrs[NL80211_ATTR_IE]);
    }

    if (info->attrs[NL80211_ATTR_USE_MFP]) {
        connect.mfp = (typeof(connect.mfp))nla_get_u32(info->attrs[NL80211_ATTR_USE_MFP]);
    ...
    if (info->attrs[NL80211_ATTR_PREV_BSSID])
        connect.prev_bssid =
        (typeof(connect.prev_bssid))nla_data(info->attrs[NL80211_ATTR_PREV_BSSID]);
    ...
    if (info->attrs[NL80211_ATTR_WIPHY_EDMG_CHANNELS]) {
        ...
        if (info->attrs[NL80211_ATTR_WIPHY_EDMG_BW_CONFIG])
            connect.edmg.bw_config =
                (typeof(connect.edmg.bw_config))nla_get_u8(info->attrs[NL80211_ATTR_WIPHY_EDMG_BW_CONFIG]);
    }
    ...
    if (wiphy_ext_feature_isset(&rdev->wiphy,
        ...
        (typeof(connect.fils_erp_username))nla_data(info->attrs[NL80211_ATTR_FILS_ERP_USERNAME]);
        connect.fils_erp_username_len =
            nla_len(info->attrs[NL80211_ATTR_FILS_ERP_USERNAME]);
        connect.fils_erp_realm =
        (typeof(connect.fils_erp_realm))nla_data(info->attrs[NL80211_ATTR_FILS_ERP_REALM]);
        ...
        (typeof(connect.fils_erp_rrk))nla_data(info->attrs[NL80211_ATTR_FILS_ERP_RRK]);
                     ...
                     struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_AUTH_TYPE]) {
        ...
        if (!nl80211_valid_auth_type(rdev, (enum nl80211_auth_type)auth_type,
                         NL80211_CMD_CONNECT))
            return -EINVAL;
        ...
        connect.auth_type = (typeof(connect.auth_type))auth_type;
...
static int nl80211_setdel_pmksa(struct sk_buff *skb, struct genl_info *info)
{
    ...
    pmksa.pmkid = (typeof(pmksa.pmkid))nla_data(info->attrs[NL80211_ATTR_PMKID]);

    if (info->attrs[NL80211_ATTR_MAC]) {
        pmksa.bssid = (typeof(pmksa.bssid))nla_data(info->attrs[NL80211_ATTR_MAC]);
    } else if (info->attrs[NL80211_ATTR_SSID] &&
           info->attrs[NL80211_ATTR_FILS_CACHE_ID] &&
           (info->genlhdr->cmd == NL80211_CMD_DEL_PMKSA ||
            info->attrs[NL80211_ATTR_PMK])) {
        pmksa.ssid = (typeof(pmksa.ssid))nla_data(info->attrs[NL80211_ATTR_SSID]);
        pmksa.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);
        pmksa.cache_id =
            (typeof(pmksa.cache_id))nla_data(info->attrs[NL80211_ATTR_FILS_CACHE_ID]);
    } else {
        return -EINVAL;
    }
    if (info->attrs[NL80211_ATTR_PMK]) {
        pmksa.pmk = (typeof(pmksa.pmk))nla_data(info->attrs[NL80211_ATTR_PMK]);
...
static int nl80211_tdls_mgmt(struct sk_buff *skb, struct genl_info *info)
{
    ...
    peer = (typeof(peer))nla_data(info->attrs[NL80211_ATTR_MAC]);
    ...
    return rdev_tdls_mgmt(rdev, dev, peer, action_code,
                  ...
                  initiator,
                  (const u8 *)nla_data(info->attrs[NL80211_ATTR_IE]),
...
static int nl80211_tdls_oper(struct sk_buff *skb, struct genl_info *info)
{
    ...
    operation = (typeof(operation))nla_get_u8(info->attrs[NL80211_ATTR_TDLS_OPERATION]);
    peer = (typeof(peer))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_register_mgmt(struct sk_buff *skb, struct genl_info *info)
{
    ...
    return cfg80211_mlme_register_mgmt(wdev, info->snd_portid, frame_type,
                       (const u8 *)nla_data(info->attrs[NL80211_ATTR_FRAME_MATCH]),
...
static int nl80211_tx_mgmt(struct sk_buff *skb, struct genl_info *info)
{
    ...
    params.buf = (typeof(params.buf))nla_data(info->attrs[NL80211_ATTR_FRAME]);
    ...
    if (info->attrs[NL80211_ATTR_CSA_C_OFFSETS_TX]) {
        ...
        params.csa_offsets =
        (typeof(params.csa_offsets))nla_data(info->attrs[NL80211_ATTR_CSA_C_OFFSETS_TX]);
                ...
                u32 hysteresis)
{
    ...
    if (n_thresholds) {
        ...
        cqm_config = (typeof(cqm_config))kzalloc(sizeof(struct cfg80211_cqm_config) +
...
static int nl80211_set_cqm(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (attrs[NL80211_ATTR_CQM_RSSI_THOLD] &&
        ...
        const s32 *thresholds =
            (typeof(thresholds))nla_data(attrs[NL80211_ATTR_CQM_RSSI_THOLD]);
...
static int nl80211_join_mesh(struct sk_buff *skb, struct genl_info *info)
{
    ...
    setup.mesh_id = (typeof(setup.mesh_id))nla_data(info->attrs[NL80211_ATTR_MESH_ID]);
    ...
    if (info->attrs[NL80211_ATTR_BSS_BASIC_RATES]) {
        u8 *rates = (typeof(rates))nla_data(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        int n_rates =
            nla_len(info->attrs[NL80211_ATTR_BSS_BASIC_RATES]);
        struct ieee80211_supported_band *sband;
                    ...
                    struct cfg80211_wowlan *trig)
{
    ...
    if (tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_TOKEN]) {
        ...
        tok = (typeof(tok))nla_data(tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_TOKEN]);
    ...
    if (tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_SEQ]) {
        seq = (typeof(seq))nla_data(tb[NL80211_WOWLAN_TCP_DATA_PAYLOAD_SEQ]);
    ...
    cfg = (typeof(cfg))kzalloc(size, GFP_KERNEL);
    if (!cfg)
        return -ENOMEM;
                   ...
                   struct cfg80211_wowlan *trig)
{
    ...
    tb = (typeof(tb))kcalloc(NUM_NL80211_ATTR, sizeof(*tb), GFP_KERNEL);
    if (!tb)
        return -ENOMEM;
...
static int nl80211_set_wowlan(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]) {
        ...
        new_triggers.patterns = (typeof(new_triggers.patterns))kcalloc(n_patterns,
        ...
        nla_for_each_nested(pat, tb[NL80211_WOWLAN_TRIG_PKT_PATTERN],
            ...
            mask_pat = (typeof(mask_pat))kmalloc(mask_len + pat_len, GFP_KERNEL);
            if (!mask_pat) {
                err = -ENOMEM;
                goto error;
            }
    ...
    ntrig = (typeof(ntrig))kmemdup(&new_triggers, sizeof(new_triggers), GFP_KERNEL);
    if (!ntrig) {
        err = -ENOMEM;
        goto error;
    }
                       ...
                       struct cfg80211_coalesce_rules *new_rule)
{
    ...
    if (tb[NL80211_ATTR_COALESCE_RULE_CONDITION])
        new_rule->condition =
            (typeof(new_rule->condition))nla_get_u32(tb[NL80211_ATTR_COALESCE_RULE_CONDITION]);
    ...
    new_rule->patterns = (typeof(new_rule->patterns))kcalloc(n_patterns, sizeof(new_rule->patterns[0]),
                     GFP_KERNEL);
...
static int nl80211_set_coalesce(struct sk_buff *skb, struct genl_info *info)
{
    ...
    new_coalesce.rules = (typeof(new_coalesce.rules))kcalloc(n_rules, sizeof(new_coalesce.rules[0]),
                     GFP_KERNEL);
    ...
    n_coalesce = (typeof(n_coalesce))kmemdup(&new_coalesce, sizeof(new_coalesce), GFP_KERNEL);
    if (!n_coalesce) {
        err = -ENOMEM;
        goto error;
    }
...
static int nl80211_set_rekey_data(struct sk_buff *skb, struct genl_info *info)
{
    ...
    rekey_data.kek = (typeof(rekey_data.kek))nla_data(tb[NL80211_REKEY_DATA_KEK]);
    rekey_data.kck = (typeof(rekey_data.kck))nla_data(tb[NL80211_REKEY_DATA_KCK]);
    rekey_data.replay_ctr = (typeof(rekey_data.replay_ctr))nla_data(tb[NL80211_REKEY_DATA_REPLAY_CTR]);
                ...
                struct genl_info *info)
{
    ...
    addr = (typeof(addr))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_register_beacons(struct sk_buff *skb, struct genl_info *info)
{
    ...
    nreg = (typeof(nreg))kzalloc(sizeof(*nreg), GFP_KERNEL);
    if (!nreg)
        return -ENOMEM;
                 ...
                 bool tx)
{
    ...
    filter = (typeof(filter))kcalloc(n_entries, sizeof(*func->rx_filters), GFP_KERNEL);
    if (!filter)
        return -ENOMEM;

    i = 0;
    nla_for_each_nested(attr, attr_filter, rem) {
        filter[i].filter = (typeof(filter[i].filter))nla_memdup(attr, GFP_KERNEL);
                ...
                struct genl_info *info)
{
    ...
    func = (typeof(func))kzalloc(sizeof(*func), GFP_KERNEL);
    if (!func)
        return -ENOMEM;
    ...
    func->type = (typeof(func->type))nla_get_u8(tb[NL80211_NAN_FUNC_TYPE]);
    ...
    if (tb[NL80211_NAN_FUNC_SERVICE_INFO]) {
        ...
        func->serv_spec_info =
            (typeof(func->serv_spec_info))kmemdup(nla_data(tb[NL80211_NAN_FUNC_SERVICE_INFO]),
                func->serv_spec_info_len,
                GFP_KERNEL);
    ...
    if (tb[NL80211_NAN_FUNC_SRF]) {
        ...
        if (srf_tb[NL80211_NAN_SRF_BF]) {
            ...
            func->srf_bf =
                (typeof(func->srf_bf))kmemdup(nla_data(srf_tb[NL80211_NAN_SRF_BF]),
            ...
            func->srf_macs =
                (typeof(func->srf_macs))kcalloc(n_entries, sizeof(*func->srf_macs),
                    GFP_KERNEL);
            if (!func->srf_macs) {
                err = -ENOMEM;
                goto out;
...
static int nl80211_update_ft_ies(struct sk_buff *skb, struct genl_info *info)
{
    ...
    ft_params.ie = (typeof(ft_params.ie))nla_data(info->attrs[NL80211_ATTR_IE]);
                       ...
                       struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_CRIT_PROT_ID])
        proto = (typeof(proto))nla_get_u16(info->attrs[NL80211_ATTR_CRIT_PROT_ID]);
...
static int nl80211_vendor_cmd(struct sk_buff *skb, struct genl_info *info)
{
    ...
    for (i = 0; i < rdev->wiphy.n_vendor_commands; i++) {
        ...
        if (info->attrs[NL80211_ATTR_VENDOR_DATA]) {
            data = (typeof(data))nla_data(info->attrs[NL80211_ATTR_VENDOR_DATA]);
                   ...
                   struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_QOS_MAP]) {
        pos = (typeof(pos))nla_data(info->attrs[NL80211_ATTR_QOS_MAP]);
        len = nla_len(info->attrs[NL80211_ATTR_QOS_MAP]);

        if (len % 2)
            return -EINVAL;

        qos_map = (typeof(qos_map))kzalloc(sizeof(struct cfg80211_qos_map), GFP_KERNEL);
        if (!qos_map)
            return -ENOMEM;
...
static int nl80211_set_pmk(struct sk_buff *skb, struct genl_info *info)
{
    ...
    pmk_conf.aa = (typeof(pmk_conf.aa))nla_data(info->attrs[NL80211_ATTR_MAC]);
    if (memcmp(pmk_conf.aa, wdev->current_bss->pub.bssid, ETH_ALEN)) {
        ret = -EINVAL;
        goto out;
    }

    pmk_conf.pmk = (typeof(pmk_conf.pmk))nla_data(info->attrs[NL80211_ATTR_PMK]);
    ...
    if (info->attrs[NL80211_ATTR_PMKR0_NAME])
        pmk_conf.pmk_r0_name =
            (typeof(pmk_conf.pmk_r0_name))nla_data(info->attrs[NL80211_ATTR_PMKR0_NAME]);
...
static int nl80211_del_pmk(struct sk_buff *skb, struct genl_info *info)
{
    ...
    wdev_lock(wdev);
    aa = (typeof(aa))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_external_auth(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_PMKID])
        params.pmkid = (typeof(params.pmkid))nla_data(info->attrs[NL80211_ATTR_PMKID]);
...
static int nl80211_tx_control_port(struct sk_buff *skb, struct genl_info *info)
{
    ...
    buf = (typeof(buf))nla_data(info->attrs[NL80211_ATTR_FRAME]);
    len = nla_len(info->attrs[NL80211_ATTR_FRAME]);
    dest = (typeof(dest))nla_data(info->attrs[NL80211_ATTR_MAC]);
...
static int nl80211_update_owe_info(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NL80211_ATTR_IE]) {
        owe_info.ie = (typeof(owe_info.ie))nla_data(info->attrs[NL80211_ATTR_IE]);
              ...
              struct genl_info *info, const u8 *peer)
{
    ...
    if (attrs[NL80211_TID_CONFIG_ATTR_NOACK]) {
        ...
        tid_conf->noack =
            (typeof(tid_conf->noack))nla_get_u8(attrs[NL80211_TID_CONFIG_ATTR_NOACK]);
    }
    ...
    if (attrs[NL80211_TID_CONFIG_ATTR_AMPDU_CTRL]) {
        ...
        tid_conf->ampdu =
            (typeof(tid_conf->ampdu))nla_get_u8(attrs[NL80211_TID_CONFIG_ATTR_AMPDU_CTRL]);
    }

    if (attrs[NL80211_TID_CONFIG_ATTR_RTSCTS_CTRL]) {
        tid_conf->mask |= BIT(NL80211_TID_CONFIG_ATTR_RTSCTS_CTRL);
        tid_conf->rtscts =
            (typeof(tid_conf->rtscts))nla_get_u8(attrs[NL80211_TID_CONFIG_ATTR_RTSCTS_CTRL]);
    }

    if (attrs[NL80211_TID_CONFIG_ATTR_AMSDU_CTRL]) {
        tid_conf->mask |= BIT(NL80211_TID_CONFIG_ATTR_AMSDU_CTRL);
        tid_conf->amsdu =
            (typeof(tid_conf->amsdu))nla_get_u8(attrs[NL80211_TID_CONFIG_ATTR_AMSDU_CTRL]);
    }

    if (attrs[NL80211_TID_CONFIG_ATTR_TX_RATE_TYPE]) {
        u32 idx = NL80211_TID_CONFIG_ATTR_TX_RATE_TYPE, attr;

        tid_conf->txrate_type = (typeof(tid_conf->txrate_type))nla_get_u8(attrs[idx]);

        if (tid_conf->txrate_type != NL80211_TX_RATE_AUTOMATIC) {
            attr = NL80211_TID_CONFIG_ATTR_TX_RATE;
            err = nl80211_parse_tx_bitrate_mask(info, attrs, (enum nl80211_attrs)attr,
                  ...
                  struct genl_info *info)
{
    ...
    tid_config = (typeof(tid_config))kzalloc(struct_size(tid_config, tid_conf, num_conf),
                 GFP_KERNEL);
    if (!tid_config)
        return -ENOMEM;
    ...
    if (info->attrs[NL80211_ATTR_MAC])
        tid_config->peer = (typeof(tid_config->peer))nla_data(info->attrs[NL80211_ATTR_MAC]);
                  ...
                  struct genl_info *info)
{
    if (info->user_ptr[1]) {
        if (ops->internal_flags & NL80211_FLAG_NEED_WDEV) {
            ...
            dev_put((struct net_device *)info->user_ptr[1]);
        }
    }
                  ...
                  size_t len)
{
    ...
    struct wiphy *wiphy = wdev->wiphy;
    struct cfg80211_registered_device *rdev = wiphy_to_rdev(wiphy);
    const struct ieee80211_mgmt *mgmt = (typeof(mgmt))buf;
    u32 cmd;
    ...
    nl80211_send_mlme_event(rdev, dev, buf, len, (enum nl80211_commands)cmd, GFP_ATOMIC, -1,
                NULL, 0);
}
...
{
    void **cb = (void **)msg->cb;
    struct cfg80211_registered_device *rdev = (typeof(rdev))cb[2];

    nla_nest_end(msg, (struct nlattr *)cb[1]);
                 ...
                 u32 rate, u32 intvl, gfp_t gfp)
{
    ...
    msg = cfg80211_prepare_cqm(dev, (const char *)peer, gfp);
    if (!msg)
        return;
...
static int nl80211_netlink_notify(struct notifier_block * nb,
                  ...
                  void *_notify)
{
    struct netlink_notify *notify = (typeof(notify))_notify;
    struct cfg80211_registered_device *rdev;
    struct wireless_dev *wdev;
