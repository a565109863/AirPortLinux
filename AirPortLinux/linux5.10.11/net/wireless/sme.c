...
struct cfg80211_conn {
	...
	enum CFG80211_CONN_STATE state;
...
static int cfg80211_conn_scan(struct wireless_dev *wdev)
{
	...
	request = (struct cfg80211_scan_request *)kzalloc(sizeof(*request) + sizeof(request->ssids[0]) +
				     ...
				     const u8 **out_ies, size_t *out_ies_len)
{
	...
	if (!rdev->wiphy.extended_capabilities_len ||
		...
		*out_ies = (const u8 *)kmemdup(ies, ies_len, GFP_KERNEL);
		if (!*out_ies)
			return -ENOMEM;
		*out_ies_len = ies_len;
		return 0;
	...
	buf = (u8 *)kmalloc(ies_len + rdev->wiphy.extended_capabilities_len + 2,
		      GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
				...
				const u8 *prev_bssid)
{
	...
	wdev->conn = (struct cfg80211_conn *)kzalloc(sizeof(*wdev->conn), GFP_KERNEL);
	if (!wdev->conn)
		return -ENOMEM;
			       ...
			       bool wextev)
{
	...
	country_ie = (const u8 *)kmemdup(country_ie, 2 + country_ie[1], GFP_ATOMIC);
	rcu_read_unlock();
			   ...
			   gfp_t gfp)
{
	...
	ev = (struct cfg80211_event *)kzalloc(sizeof(*ev) + (params->bssid ? ETH_ALEN : 0) +
		     ...
		     gfp_t gfp)
{
	...
	ev = (struct cfg80211_event *)kzalloc(sizeof(*ev) + info->req_ie_len + info->resp_ie_len +
			      ...
			      gfp_t gfp)
{
	...
	ev = (struct cfg80211_event *)kzalloc(sizeof(*ev), gfp);
	if (!ev)
		return;
			   ...
			   bool locally_generated, gfp_t gfp)
{
	...
	ev = (struct cfg80211_event *)kzalloc(sizeof(*ev) + ie_len, gfp);
	if (!ev)
		return;
