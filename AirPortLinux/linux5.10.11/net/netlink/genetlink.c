...
{
    return (struct genl_family *)idr_find(&genl_fam_idr, id);
}
    ...
    do {
        ...
        if (id + n_groups > mc_groups_longs * BITS_PER_LONG) {
            ...
            if (mc_groups == &mc_group_start) {
                new_groups = (typeof(new_groups))kzalloc(nlen, GFP_KERNEL);
                if (!new_groups)
                    return -ENOMEM;
                mc_groups = new_groups;
                *mc_groups = mc_group_start;
            } else {
                new_groups = (typeof(new_groups))krealloc(mc_groups, nlen,
                              GFP_KERNEL);
                if (!new_groups)
                    return -ENOMEM;
                mc_groups = new_groups;
...
{
    return (struct genl_dumpit_info *)kmalloc(sizeof(struct genl_dumpit_info), GFP_KERNEL);
}
...
{
    enum netlink_validation validate = (typeof(validate))(ops->validate & no_strict_flag ?
                       NL_VALIDATE_LIBERAL :
                       NL_VALIDATE_STRICT);
    ...
    attrbuf = (typeof(attrbuf))kmalloc_array(ops->maxattr + 1,
                sizeof(struct nlattr *), GFP_KERNEL);
    if (!attrbuf)
        return (struct nlattr **)ERR_PTR(-ENOMEM);

    err = __nlmsg_parse(nlh, hdrlen, attrbuf, ops->maxattr, ops->policy,
                validate, extack);
    if (err) {
        kfree(attrbuf);
        return (struct nlattr **)ERR_PTR(err);
    }
    return attrbuf;
}
...
{
    struct genl_start_context *ctx = (typeof(ctx))cb->data;
                    ...
                    int hdrlen, struct net *net)
{
    ...
    info.nlhdr = nlh;
    info.genlhdr = (typeof(info.genlhdr))nlmsg_data(nlh);
    info.userhdr = (char *)nlmsg_data(nlh) + GENL_HDRLEN;
    info.attrs = attrbuf;
    info.extack = extack;
                   ...
                   struct netlink_ext_ack *extack)
{
    ...
    struct genlmsghdr *hdr = (typeof(hdr))nlmsg_data(nlh);
    struct genl_ops op;
    int hdrlen;
                         ...
                         u32 portid, int seq, u8 cmd)
{
    ...
    if (skb == NULL)
        return (struct sk_buff *)ERR_PTR(-ENOBUFS);
...
static int ctrl_getfamily(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[CTRL_ATTR_FAMILY_NAME]) {
        ...
        name = (typeof(name))nla_data(info->attrs[CTRL_ATTR_FAMILY_NAME]);
...
{
    const struct genl_dumpit_info *info = genl_dumpit_info(cb);
    struct ctrl_dump_policy_ctx *ctx = (typeof(ctx))cb->ctx;
    ...
    if (tb[CTRL_ATTR_FAMILY_ID]) {
        ...
        rt = genl_family_find_byname(
            (char *)nla_data(tb[CTRL_ATTR_FAMILY_NAME]));
        if (!rt)
            return -ENOENT;
        ctx->fam_id = rt->id;
    }
...
//                              op.maxattr);
    ...
    for (i = 0; i < genl_get_cmd_cnt(rt); i++) {
        ...
        if (op.policy) {
            err = 0;//netlink_policy_dump_add_policy(&ctx->state,
//                                 op.policy,
//                                 op.maxattr);
            if (err)
                return err;
...
static void genl_pernet_exit(struct net *net)
{
